...
3. **Dispatch by node** — present a **two-option** prompt, never a bare yes/no:
   ...
   | `revising` | the revision-loop dispatcher — `../skills/revision-coach/SKILL.md` §Loop Dispatch ("What now?") proposes the next leverage action from the finding lifecycle; the stored `next_action` (`revision_round`/`coaching`) is its default. **Before entering Loop Dispatch**, check round-trip eligibility and, when eligible, add the round-trip resume offer — see the round-trip note below |
   | `diagnosed` | offer `/coach`; **also** offer *"regenerate the marked-up copy from this run?"* — see note below; **also** offer *"regenerate the reader-contract reverse outline from this run?"* — see note below; **also** add the round-trip resume offer when eligible — see the round-trip note below |
   ...
   **`diagnosed` — Reader-Contract Reverse Outline re-generation (no new command).** A second *sibling* offer alongside `/coach`, on the same terms as the Annotated-Manuscript one — *"regenerate the reader-contract reverse outline from this run?"* — **only when** the resolved run folder holds a `*_…_DE_Synthesis_*` letter, a `*_Pass0_Reverse_Outline_*`, and a `*_Contract_*` **and no** `*_Reader_Contract_Outline_*.md`. The condition is this **no-outline glob (plus the three input globs), not** a `next_action` value (do not invent one). On **yes**, run the generation chain in `../skills/core-editor/references/run-synthesis.md §Reader-Contract Reverse Outline` (author the Contract Map → `reader_contract_outline.py build` R7-gates it → project the outline → `validate.sh reader-contract-outline` R1–R7, staged in a temp copy). **Map state at regeneration (explicit branch):** if the run folder already holds a `*_Contract_Map_*` whose `inputs.*_sha256` **match** the run folder's Pass 0 / Contract / Ledger, **reuse it frozen** (the rebuild is byte-identical); if the Map is **absent** (a pre-wiring run), **re-author + re-gate** it before rebuilding; if the Map is present but **stale** (any input hash mismatch), **fail loud** — name the drifted input and stop, never silently re-author over a frozen Map (the stale-`sha256` rule from the Annotation Manifest). If any of the three inputs is missing on an explicit regeneration request, **fail loud** — name the absent input, write nothing. If **several** diagnosed runs qualify, list them and let the author pick — never regenerate against an unnamed run silently.

   **`revising`/`diagnosed` — Round-trip resume (no new command).** The built round-trip revision loop (`state-lifecycle.md` §Round-Trip Re-Anchoring) surfaces here as a one-click resume. **Eligibility (mechanical, no new state):** `python3` is available (`command -v python3`) **and** a prior run folder under the project root holds a `*_Annotation_Manifest_*.md` (the `annotation_manifest._MANIFEST_GLOB`; an emitted `*_Reanchored_Manifest_*` is excluded by name, so a carried-over manifest never masquerades as a prior diagnosis). If **several** run folders qualify, list them and let the author pick (the multiple-runs rule above) — never diff against an unnamed run silently. When eligible, the node prompt becomes (two-option discipline, never a bare yes/no): **"Returning with a revised draft?"** → the round-trip chain — `../skills/core-editor/references/state-lifecycle.md` §Round-Trip Re-Anchoring, run in order: the §When to Reset check **first** (a rewrite routes to fresh full analysis, not the round-trip), then new run folder + snapshot → `reanchor` classify → `emit -o <new_run_folder>` (**exit (a) — carry-only**: the writer may stop with the revision-aware marked-up copy; nothing touches finding state), optionally targeted re-diagnosis → `regression-diff` + `crossref` → step 4 **Disposition & round-close** (**exit (b)**). **"Continuing to work the plan?"** (at `revising`) → §Loop Dispatch exactly as today, the stored `next_action` its default; at `diagnosed` the existing `/coach` + sibling offers stand. The condition is this **manifest glob, not** a `next_action` value (do not invent one). When ineligible — no prior annotated run, or no `python3` — the row behaves exactly as today (the manual Revision Round Protocol remains the documented path). **Disposition display:** at these nodes the resume summary also names any **active finding dispositions** (`execution.finding_dispositions`, markers as fallback; active = not superseded by `revised` — `state-lifecycle.md` §Finding Dispositions), e.g. *"2 declined, 1 deferred — trigger 'beta reads back' not yet fired"* — display only; the round-trip proposal step never proposes `declined`/`deferred` (author-initiated only).
