...
3. **Dispatch by node** — present a **two-option** prompt, never a bare yes/no:
   ...
   | `diagnosed` | offer `/coach`; **also** offer *"regenerate the marked-up copy from this run?"* — see note below; **also** add the round-trip resume offer when eligible — see the round-trip note below |
