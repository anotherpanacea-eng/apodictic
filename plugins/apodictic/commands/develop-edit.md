...
4. **Synthesis:** Identify 3-5 root causes, triage into Must-Fix / Should-Fix / Could-Fix, generate revision checklist (max 10 items), revision order, and top 10 reader questions. Verify every flag against stated intent before finalizing.

5. **Evaluate Full DE trigger:** If >5 root causes, >10 major reader experience issues, or structural complexity is high, offer to run Full DE passes (3, 4, 6, 7, 9, 10) and supplementary audits. Load `references/run-full.md` only when this trigger is met.

6. **Output:** Write all artifacts to the project's `Outputs/` folder using the naming convention `[Project]_[PassName]_[runlabel].md`.
