...
- **Dynamic move signals — SHIP as `agd_move_scan` located observations (observed, not pinned).** The two dynamic collapse signals this note once deferred (the disappearing-guard and the discounting/straw-man moves) now ship — not on *this* surface's aggregate, but as SETEC's separate **`agd_move_scan`** surface (min 1.124.0; `handoff: experimental`, `calibration_status: heuristic`), which reports LOCATED, verbatim-anchored ASSURING/GUARDING/DISCOUNTING move observations — **unanchored and aggregate-excluded** (the surface emits no threshold, score, count, or aggregate). APODICTIC consumes them through the **AGD Move Audit** (`argument-agd`; Argument_State §10.9's scan pre-pass), on the same **observed-not-pinned** posture as the B3/B4 `reused_signals` above (§131): descriptive location context only — never an anchored contribution, never in any aggregate, never a direction claim. Each observation is a POINTER; every challenge, result, and code is the audit's (R4A ADR D5).
