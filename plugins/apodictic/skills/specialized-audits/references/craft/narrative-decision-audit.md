...
The surface emits the same top-level envelope APODICTIC already parses for `variance_audit`, `voice_distance`, etc. The floor is now **data-driven from the capabilities manifest**, not hardcoded: resolve it with `setec_capabilities.resolve_floor("narrative_decision_audit")` (which discovers SETEC, queries `capabilities emit --json`, and asserts the discovered `setec_version` ≥ the surface's manifest `min_setec_version`), then invoke through the pass-side helper with the validated location — `setec_runner.run_supplement("narrative_decision_audit.py", args, location=manifest.location)` — and read from `result.results`. (The direct shim `ai_prose_narrative_decision_audit.py` does exactly this for CLI use; a SETEC below the surface's manifest floor fails with an upgrade message naming the surface's required minimum rather than a missing-script error.) The load-bearing keys:
