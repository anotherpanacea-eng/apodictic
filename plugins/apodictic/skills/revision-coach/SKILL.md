...
**Inputs (all already on disk):** `Diagnostic_State.meta.json` — `execution.finding_states` (`locked`/`delivered`/`revised`), `execution.finding_dispositions` (the declined/deferred overlay records, `docs/finding-dispositions.md`; the pinned disposition markers in the Coaching Log / Revision Report / Feedback Triage artifact are the fallback when the map lags), `execution.pending_gate` / `phase` / `allowed_next` (the gate frontier, for step 1), `revision_progress`, `triage_summary`, `control_questions.open`, the Coaching Log — and the `<!-- resolved: F-… -->` markers in the latest revision report.
...
3. A Must-Fix is `delivered` but not `revised` → the highest-leverage open fix → **Session Planning** scoped to it, or an execution-mode handoff for scene-level work. *(The loop's core.)* **Disposition overlay** (`docs/finding-dispositions.md`; active = `finding_states[<id>] != "revised"`): the selector **skips** ids with an active `declined` disposition; for an active `deferred` id, run a **trigger review** first — a trigger that parses as an ISO date (`YYYY-MM` / `YYYY-MM-DD`) fires mechanically once the date has passed; any other trigger text is a coach judgment call against the session context ("you deferred F-P5-03 until 'after the POV decision' — that decision is now recorded; resurface it?"). A fired deferral re-enters this step at normal leverage rank; an unfired one is skipped. If **every** open Must-Fix is declined or deferred-unfired, fall through to step 4 (the Should-Fix tier) with a one-line note naming the set-aside findings.
   - **Stalled off-ramp:** if that finding has seen no progress across multiple sessions (Coaching Log) and still has no resolved marker, don't re-offer the same plan — surface "this finding has stalled: try an execution-mode handoff, or set it aside." On "set it aside," **record the disposition**: offer `declined` vs `deferred` + trigger explicitly and let the writer choose — never choose for them (Firewall: the dispatcher proposes, the writer disposes). Write the pinned marker in the Coaching Log (`<!-- declined: F-… — <reason> -->` / `<!-- deferred: F-… until: <trigger> — <reason> -->`) **and** record the engine disposition via the dual-writer path (`../core-editor/references/state-lifecycle.md` §Finding Dispositions): non-governed project → validate the record and write `execution.finding_dispositions[<id>]` directly; runner-governed → note the marker folds at the next `revision_round` clear. A later session then sees a deliberate pass, not an oversight.
4. All Must-Fix `revised`, Should-Fix still open → **Session Planning** at the next severity tier.
5. All findings `revised` **or actively dispositioned**, and `control_questions.open == 0` → offer **submission readiness** (`/ready`). Declined Must-Fixes will be listed, with reasons, in the readiness verdict; they are not absorbed.
...
**Provenance you MUST persist + the honesty caveat (the F2 + P1 rule — per pattern).** Whether H2 can *verify* a streak against a record differs by pattern, and any streak that isn't independently verified MUST carry a visible honesty caveat so the writer never sees an unverified pattern presented as if it were checked (H2 flags a missing caveat: WARN, ERROR under `--strict`):
- **`deferral-recurrence`** — verifiable. On a **runner-governed** project H2 reads the independent `execution.gate_events[].disposition_deltas` event log (nothing extra to do; no caveat needed). On a **non-governed** project the folded `execution.finding_dispositions` map is last-event-wins, so when you record a `deferred` disposition you **must persist the full session history** — append the session ordinal to a `sessions: [<n>, …]` list on `execution.finding_dispositions[<F-id>]` (without it a genuine 3-session streak fails H2 as unresolved) — **and** carry the caveat (the `sessions` list is self-reported, not an independently checked event log).
- **`phase-incompletion`** — **not verifiable against any record on any project** (per-session revision-arc-phase completion is persisted nowhere — the arc is a stateless overwriting re-plan, `finding_states` is a rolling last-write map). So it is self-reported **everywhere** and **ALWAYS** requires the caveat — governed or not.

Caveat phrasing: say plainly the streak is from *your own running session notes*, e.g. "from what I've been tracking across our sessions, …". Do NOT dress a self-reported streak as a checked fact.
