...
**Provenance you MUST persist so H2 can verify the streak (the F2 rule).** H2's anti-fabrication proof needs the per-session deferral history. On a **runner-governed** project it reads the independent `execution.gate_events[].disposition_deltas` event log — nothing extra to do. On a **non-governed** project the folded `execution.finding_dispositions` map is last-event-wins (it remembers only the latest deferred session), so when you record a `deferred` disposition you **must persist the full session history on the record** — add the deferral's session ordinal to a `sessions: [<n>, …]` list on `execution.finding_dispositions[<F-id>]` (append at each deferral). Without it a genuine 3-session streak fails H2 as unresolved. **And because that history is self-reported (not an independently checked event log), a non-governed `deferral-recurrence` observation MUST carry a visible honesty caveat** — say plainly that the streak is from *your own running session notes*, e.g. "from what I've been tracking across our sessions, …" — so the writer never sees an unverified pattern presented as if it were checked (H2 flags a missing caveat: WARN, ERROR under `--strict`). Governed observations need no caveat (the event records ARE the verification).
