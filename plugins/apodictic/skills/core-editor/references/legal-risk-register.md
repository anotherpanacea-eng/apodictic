...
- **`defamation`** — a *factual* assertion (verifiable, not opinion) lowering an identifiable **living** person's or active entity's reputation: crime, fraud, sexual misconduct, professional incompetence, addiction, financial wrongdoing. Identifiable even if unnamed (role + place + a unique event). Watch rumor/hearsay framing ("everyone knew", "allegedly" — repetition carries the same liability) and changed-name fiction that stays recognizable. **Not a flag:** the dead, government bodies, pure opinion/hyperbole.
- **`privacy`** — true-but-intimate facts about an identifiable **living private** person (medical/psychiatric, sexual, addiction, abuse, finances, family secrets); **intrusion** admissions (secret recording, hacking, trespass — flag the *method*); **false light** (invented/embellished scene creating an offensive false impression). **Not a flag:** facts lawfully in the public record, or about a public figure germane to their public role.
- **`rights-clearance`** — quoted **song lyrics (any length)**, poetry, **epigraphs**, unpublished letters/diaries, images, or substantial prose excerpts without permission; a brand in the **title/cover/marketing** or implying endorsement; an admitted **NDA / settlement / confidentiality** covering the disclosed material; operative **trade-secret** detail; a real person's identity in **advertising/merchandise**. **Not a flag:** titles/short phrases/facts (uncopyrightable), nominative brand mention in narrative, or writing *about* a real person in expressive content (right of publicity protects expressive works — the risk is marketing/merch use).
- **`other`** — exposure outside the three: **incitement** (step-by-step instructions to commit a violent crime); commentary on an **active proceeding** or breach of a court/suppression order (UK/AU — no US analogue); **IIED**-grade targeted humiliation of a private person.

**Severity = base tier + modifiers.** Set `severity` to the trigger's base tier (below), then **raise** one step per modifier that fires (cap `review-now`; never silently lower), and name the modifiers in `escalation_trigger`: `+identifiable-living-private-person` · `+serious-allegation` (crime/sexual/professional-ruin) · `+weak-or-no-documentation` · `+international-distribution` (UK/EU/AU) · `+author-signed-agreement` · `+marketing/cover/merchandise-use` · `+minor-or-vulnerable-subject`.

**Route-to-counsel (`review-now`) bright lines.** A crime/sexual/professional-ruin allegation of fact about an identifiable living person with no public-record source · intimate non-public facts about an identifiable private person (esp. minor/victim/patient) without consent · unlicensed lyrics/poetry/unpublished writing/images · NDA- or settlement-covered disclosure · identity in advertising/merchandise · material obtained unlawfully · breach of a court/suppression order · instructions enabling a violent crime.
...
| `ROP_EXPRESSIVE` | monitor | identity within expressive content (protected) |
