...
- **Tension bleed threshold:** 3+ consecutive chapters below mid-intensity after the first wrongness → flag "Tension Dissipation — dread is leaking"
- **Relief ratio:** Aim for roughly 1 valley scene per 2–3 peak scenes in the escalation arc. More than 4 consecutive peak scenes = fatigue risk; more than 3 consecutive valleys post-wrongness = momentum loss
...
**The Certainty-Intensity Sync Check:**

Flag scenes where Certainty and Intensity move in the same direction simultaneously — UNLESS the divergence is intentional:

| Certainty ↓ + Intensity ↓ | Intentional? | Flag? |
|---------------------------|--------------|-------|
| False calm before escalation | Yes, if brief | No |
| Extended lull post-revelation | Depends on contract | **Check: "Deflation Risk"** |

| Certainty ↑ + Intensity ↑ | Intentional? | Flag? |
|---------------------------|--------------|-------|
| Clarity increases the horror | Yes (body horror, cosmic reveal) | No |
| Explanation makes threat concrete | Yes, at climax | No |
| Knowledge kills dread | No | **Yes: "Overexplanation — certainty is neutralizing the horror"** |

**Best pattern for psychological horror:** Certainty trends down while Intensity trends up. The less the character (and reader) can trust, the scarier each new event becomes.

**Thresholds:**
- **Certainty stasis:** If certainty axis holds stable for >25% of the horror arc (post-first-wrongness), flag "Reality Too Stable — nothing is genuinely uncertain"
- **Premature certainty collapse:** If certainty drops to minimum before the 40% mark, flag "Nowhere Left to Fall — destabilization peaked too early"
- **Intensity plateau:** Same intensity level sustained for 3+ chapters → flag "Dread Plateau — horror is flatlining"
...
**HOR-DD1: The "Explanation Kills" Problem**
- *Detection:* Horror source is fully explained, and dread immediately drops in the next scene. Explanation provides a mechanism that makes the threat feel solvable or mundane.
- *Test:* After the explanation, does the reader still fear the threat? Or does understanding neutralize it?
- *Threshold:* If >50% of mystery/ambiguity is resolved before the final 20% of the manuscript, flag unless the horror source shifts to a new axis (e.g., from "what is it?" to "what does it want from me?").
- *Flag:* "Premature Demystification — the horror became a problem to solve instead of a dread to endure"
- *Exception:* Body horror and domestic horror where knowing exactly what's happening IS the horror. Cosmic horror where partial explanation deepens the unknowable.

**HOR-DD2: The "Numb Protagonist" Problem**
- *Detection:* Protagonist witnesses or experiences 3+ horror events without measurable psychological deterioration. Internal monologue stays functional. Coping mechanisms don't degrade.
- *Test:* Compare protagonist's internal voice in first horror scene vs. third. If register, coherence, and emotional range are unchanged, flag.
- *Threshold:* By the 60% mark, protagonist should show at least 2 of: degraded coping, altered perception, relationship damage, identity questioning, physical symptoms.
- *Flag:* "Horror Without Cost — protagonist is experiencing horror tourism, not horror"
- *Exception:* Flat-arc horror where protagonist's stability IS the horror (they should be breaking but aren't — and the narrative frames this as wrong). Must be clearly intentional.

**HOR-DD3: The "Diminishing Returns" Escalation**
- *Detection:* Same horror technique (jump scare, body transformation, reality slip, etc.) used 3+ times without variation in mechanism, scale, or stakes.
- *Test:* List every horror beat by technique type. If any single technique appears >3 times, check: does each instance raise the stakes, change the rules, or reveal new information? If not, flag.
- *Threshold:* 3+ repetitions of same technique without escalation = flag. 5+ repetitions = "Horror Wallpaper — scary things are now scenery"
- *Flag:* "Technique Repetition — reader has habituated to this particular flavor of horror"
- *Exception:* Deliberate escalation-by-repetition where each instance is identical but the context has changed enough to alter meaning (rare, must be clearly intentional).

**HOR-DD4: The "No Normalcy Baseline" Problem**
- *Detection:* Horror elements appear within the first 10% of manuscript without establishing what "normal" looks like for the protagonist.
- *Test:* Can the reader articulate what the protagonist's life looks like when nothing is wrong? If the manuscript opens with wrongness, does it provide normalcy through flashback, contrast, or another character's perspective?
- *Threshold:* If zero scenes of established normalcy exist before the first wrongness signal, flag. Minimum: reader must understand what's at stake before it's threatened.
- *Flag:* "Missing Baseline — horror needs a 'before' to make the 'after' land"
- *Exception:* In medias res openings where normalcy is established retroactively (flashbacks, memory, contrast). Must happen within the first 20%.
...
- **Skipped Rung:** Manuscript jumps from Unease (1) to Direct Confrontation (7) without intermediate stages → Flag "Unearned Horror — dread not built, just deployed"
- **Stuck on a Rung:** Same Dread Ladder stage for 3+ chapters without escalation or meaningful variation → Flag "Dread Plateau — horror is treading water"
- **Premature Point of No Return:** Stage 6 reached before the 40% mark → Flag "Early Commitment — nowhere left to escalate" (unless contract is nihilistic horror where the long aftermath IS the story)
- **Missing Normalcy:** Stage 1 begins without any prior scene establishing what "normal" looks like → Flag "Missing Baseline" (see HOR-DD4)
- **Regression Without Purpose:** Manuscript drops 2+ rungs without narrative justification (e.g., protagonist suddenly feels safe with no reason) → Flag "Unearned Relief — tension reset without cause"
- **Repeated Rungs:** Same rung revisited 3+ times → Check: does each revisit deepen, widen, or recontextualize? If not, flag "Escalation Stall — cycling instead of climbing"
...
Dread can be shown through:
- **Sensory wrongness:** Details that are slightly off (smell, sound, temperature, light)
- **Delayed processing:** Character notices something, understands it a beat too late
- **Physical response:** Nausea, cold sweat, hair standing up, involuntary flinch, freeze response
- **Avoidance behavior:** Character stops looking at, entering, or thinking about something
- **Hypervigilance:** Noticing everything, scanning for threat, unable to relax
- **Sleep/appetite disruption:** Insomnia, nightmares, inability to eat
- **Isolation instinct:** Withdrawing from others OR desperate clinging
- **Rationalization:** Actively constructing explanations for what they know is wrong
- **Intrusive thoughts:** Unwanted images, scenarios, or memories surfacing unbidden

**Detect:** Dread asserted without demonstration through these indicators. "She felt a sense of dread" with no sensory, behavioral, or cognitive evidence = told-not-shown horror.

**Threshold:** If 3+ horror scenes rely on assertion ("felt scared," "was terrified") without at least 2 dread indicators per scene, flag "Dread Told Not Shown — the prose isn't doing the work."
