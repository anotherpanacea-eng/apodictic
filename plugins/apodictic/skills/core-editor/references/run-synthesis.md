...
9. **Conditional Underdiagnosis Retry Loop (required gate).** This step uses **enumerated detectable triggers**, not model self-judgment. Run `scripts/validate.sh underdiagnosis-triggers <editorial_letter_file>` (or perform the equivalent inline check on hosts without shell execution). The validator surfaces triggers; the model still owns the decision. For each fired trigger, you MUST either (a) upgrade the affected finding's severity in the synthesis layer, OR (b) document an override marker in the letter body (not in an appendix).
   ...
   **Enumerated triggers (detectable from artifacts; no model-judgment language):**
   ...
   7. **Softness trigger:** `validate.sh softness-check <editorial_letter> <findings_ledger>` returns ERROR or WARN — i.e., the delivered letter softens a Triage-locked finding below its locked severity (Deficit Lock, `output-policy.md §Severity Honesty Protocol`) without a body override marker.
...
10. **Pre-Output Synthesis Verification (required gate).** Before writing the letter, verify the synthesis is actually drawing from pass findings — not generating an editorial letter from general impressions of the manuscript. Run these checks:
   ...
   - **Deficit Lock / softness check:** Run `scripts/validate.sh deficit-lock <editorial_letter_file>` and `scripts/validate.sh softness-check <editorial_letter_file> <findings_ledger_file>` (or the equivalent inline check on hosts without shell execution). These compare the delivered letter against the Triage-time locked findings (Deficit Lock, canonical in `references/output-policy.md §Severity Honesty Protocol`): an unmarked downgrade of a locked Must-Fix/Should-Fix blocks delivery (ERROR); hedged delivery of a locked finding is flagged (WARN). Resolve by re-tiering, delivering at the locked severity, or recording a `<!-- override: softness-downgrade — <rationale> -->` marker in the body plus an Appendix B entry. Weak-axis-vs-Must-Fix coherence is owned separately by `severity-floor` and is not duplicated here.
