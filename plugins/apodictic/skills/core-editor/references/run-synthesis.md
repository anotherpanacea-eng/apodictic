...
9c. **Pre-Letter Re-Grounding (required — after the step-9b manifest, before any letter prose).** Before writing the letter, re-read the locked findings from disk and restore into the draft claims the exact **counts, names, and quote anchors** that context-salience decay smears into vague prose (`docs/subagent-architecture-design.md:15`: Pass 1's "nine belief failures" must not become "several belief failures" in the letter). This step runs after the Synthesis Coverage Manifest exists (step 9b) — it re-reads the consolidated ledger *and* updates that manifest — and before the Step 10 pre-output gate and Step 11 letter-writing. It is named **Pre-Letter Re-Grounding** to avoid collision with the built `run-core.md §Pre-Pass Re-Grounding` (a different seam — pre-*pass*, not pre-*letter*); **this step does not touch that section.**
   ...
   3. **Restore specificity into the draft claims** — exact counts, names, quote anchors, from the re-read ledger + text. **Re-grounding may only ADD specificity.** It may **not** add a finding (anything genuinely new noticed here goes to §4b Additional Observations, without a finding ID and without a Must-Fix/Should-Fix label, flagged "surfaced post-ledger; awaits pass-level confirmation" — the Step 10 findings-integration demotion path), **not** change any severity (Deficit Lock is untouched; severity writes from this step are forbidden — Deficit Lock + softness-check downstream remain the enforcement), and **not** remove counterevidence or soften wording (softness-check still applies at Step 10). The smuggled-finding gate is **`finding-trace`** (its E1 check: a letter citation to an F-… ID not in the ledger fails, exit 1) — the sole owner of the letter-ID-must-exist-in-ledger direction; this step names it, and does not re-implement it. As of FLI increment 4 it runs **mechanically as a row in the `run_spot_check` gate** (`schemas/execution-gates.v1.json`, Step 10), so you no longer invoke it as a separate command — E0/E1/E2/E3 block the clear (exit 1). On hosts without shell execution, perform the equivalent inline set-membership check (every cited F-… ID resolves to a ledger finding). When you write the letter (Step 11), place a `<!-- regrounding: done -->` marker on its own line **directly after the coverage marker** (which sits immediately after the title block — §1 / the step-9b V5 placement).
...
### Reader-Contract Reverse Outline (offered at run-end)

The third thing a finalized third-party developmental edit hands back (alongside the editorial letter and the Annotated Manuscript): a reverse outline that lays the book out **scene by scene** and maps it against the **reader contract** it implicitly makes — where each promise is established, where it's paid off, and where it breaks. Like the Annotated Manuscript, APODICTIC generates it as a **byte-deterministic projection** of artifacts that already exist (the Pass 0 reverse outline, the Contract, the Findings Ledger) plus a gated, ids-only **Contract Map** — the model's only authored contribution anywhere in the deliverable, closed-key and mechanically gated (R7) before the projector consumes it. See `references/reader-contract-outline.md`.

**Offer it — don't assume it, don't bury it behind a command** (a sibling offer to the Annotated Manuscript, independent of it). **Preconditions (all three, in this run folder):** a `*_…_DE_Synthesis_*` letter, a `*_Pass0_Reverse_Outline_*`, and a `*_Contract_*`. Absent any one, the offer is **silently not surfaced** — a partial-de / fragment-de / triage run legitimately lacks them (the same run-shape gate the Annotated Manuscript offer uses; the Map is **not** a precondition — it is authored as step 1 of an accepted offer).

When the preconditions hold, ask: *"Want a reader-contract reverse outline — your book scene by scene, mapped against the promise it makes to its reader?"* On **yes**, run the generation chain — **staged in a temp copy so the run folder only ever receives verified artifacts**:

1. Copy the run-folder **inputs** (the `*_Pass0_Reverse_Outline_*`, the `*_Contract_*`, and the Findings Ledger) into a scratch staging directory. Author/gate/render there, never in place.
2. **Author the Contract Map** — one `apodictic.contract_map.v1` block (`[Project]_Contract_Map_[runlabel].md`): closed-key, **ids only, no prose**, reading only the staged artifacts. Localize each contract clause (the READER PROMISE clauses, the CONTROLLING IDEA / CENTRAL QUESTION, each NON-NEGOTIABLES item — items are **`;`-separated**, the shipped contract convention; R7 recounts them from the Contract) to the Pass 0 scene ids that **establish** and **pay it off**; bind `inputs.pass0_sha256` / `contract_sha256` / `ledger_sha256` to the staged files; name a `gap_finding_id` only when a real Ledger finding covers the breach (else `null`). If the run folder already holds a `*_Contract_Map_*` bound to these same inputs, reuse it (see the regeneration branch in `commands/start.md` §Step 0.5).
3. `scripts/reader_contract_outline.py build <staging>` — **R7 gates the Map first**; on pass, it projects the outline (`[Project]_Reader_Contract_Outline_[runlabel].md`).
4. `scripts/validate.sh reader-contract-outline <staging>` — the **R1–R7** gate (the firewall, mechanical).
5. **Only if the build's R7 gate and the R1–R7 validator both pass**, move the Map + outline into the run folder, then point the author to the reverse outline. On any gate failure, report **which** gate failed and that **nothing was written** (discard staging); the build is deterministic, so re-running after a fix is clean. On hosts without shell execution, perform the author/gate/render/gate inline with the same verified-or-absent discipline.

**Scope:** fiction (the full 10-field contract schema) and narrative nonfiction (+ the optional `CENTRAL QUESTION` / `PROMISE TYPE` block, which drives the deliverable's idea clause in place of `CONTROLLING IDEA`). Argument-shaped nonfiction is deferred (a separate argument-spine analog — flag, don't build). **Re-generating later:** an author who declines here can regenerate via `/start`'s `diagnosed`-node dispatch (see `commands/start.md` §Step 0.5) — no separate command, no new `next_action`.
