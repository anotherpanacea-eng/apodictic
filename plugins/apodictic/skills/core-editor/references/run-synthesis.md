...
2. **Author the Contract Map** — one `apodictic.contract_map.v1` block (`[Project]_Contract_Map_[runlabel].md`): closed-key, **ids only, no prose**, reading only the staged artifacts. Localize each contract clause (the READER PROMISE clauses, the CONTROLLING IDEA / CENTRAL QUESTION, each NON-NEGOTIABLES item) to the Pass 0 scene ids that **establish** and **pay it off**; bind `inputs.pass0_sha256` / `contract_sha256` / `ledger_sha256` to the staged files; name a `gap_finding_id` only when a real Ledger finding covers the breach (else `null`). If the run folder already holds a `*_Contract_Map_*` bound to these same inputs, reuse it (see the regeneration branch in `commands/start.md` §Step 0.5).
