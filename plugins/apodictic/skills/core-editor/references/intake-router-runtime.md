...
| Legal Risk Register | `constraint:risk` **or** content-detection (memoir / autofiction / real people) | Adds a Legal Risk Register output (offer-then-attach) | Any repair/submit base route (`references/legal-risk-register.md`) | **Built** |
...
**Content-detection auto-recommend (Legal Risk Register, no explicit flag).** The Legal Risk Register overlay attaches on the explicit `constraint:risk` flag *and* on a **content signal** the model reads for — so it composes even when the author didn't flag the exposure. **Trigger:** the manuscript is memoir / autofiction / nonfiction portraying identifiable real people — a **mode signal** (first-person lived-experience retrospective; autofictional author-surrogate or real-person-mapping author's-note; nonfiction naming/depicting identifiable living people) *plus* at least one **real-people content signal** (a reputational statement of fact about a living person; intimate private facts about an identifiable private person; recognizable changed-name portrayal; quoted lyrics/poetry/unpublished third-party writing; NDA/settlement-covered disclosure). This is **prompt/router-only** (model-side detection, no mechanical validator arm), declared parallel to the `constraint:risk` route and consistent with the "auto-recommend before synthesis" precedent (Field Reconnaissance, Reception Risk — `pass-dependencies.md §4a/§4c`). On detection the synthesis layer **offers** the register (offer-then-attach, explicit confirm; **flag, don't practice law** — never adjudicate, never auto-produce). Operative offer + full signal list + false-positive guard: `run-synthesis.md §Content-detection auto-recommend`; detection patterns: `references/legal-risk-register.md` §Detection guidance / §Auto-recommend.
