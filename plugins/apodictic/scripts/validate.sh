...
EOF
  ...
  editor-scaffolding)
    ...
    # Delegates to scripts/editor_scaffolding.py; degrades to an advisory WARN without python3.
    ...
    if command -v python3 >/dev/null 2>&1 && [ -f "$ES_HELPER" ]; then
      if [ $# -lt 1 ]; then echo "Usage: $0 editor-scaffolding <editorial_letter|run_folder> [--strict] | editor-scaffolding --dual <editor_letter> <author_letter> [--strict] | --self-test"; exit 2; fi
