...
- **B2 — order.** The item order equals the declared key: **severity band descending** (Must-Fix > Should-Fix > Could-Fix), then **fewer *distinct* `evidence_refs` first**, then finding-id. Footprint (evidence-ref count) is a *disclosed mechanical proxy* for surface presence — the serious finding mentioned once is the one a confident read skims past — and it is only the **within-band** tiebreak, so a Must-Fix always outranks a Should-Fix regardless of footprint (the Deficit Lock is never subordinated to footprint). It is not a claim about human perception; it is an auditable ordering.
- **B3 — severity fidelity.** A severity token an item restates must match its anchored finding's locked Ledger severity; a lookup failure or a below-lock restatement is an ERROR — reordering can never launder a softened band.
