...
| ID | Severity | Rule |
|---|---|---|
| **E1 — mode + addressee** | ERROR | The mode marker is present **and** the letter carries the editor addressee: an `## Editor Brief` section (non-empty). A bare marker with no Editor Brief, or an Editor Brief with no marker on an otherwise scaffolded-looking letter, fails. |
| **E2 — blind-spot section** | ERROR | A non-empty **What You Might Have Missed** section is present (heading match, ≥1 content line). The scaffolding value-add cannot be silently dropped. |
| **E3 — prescription deferral** | ERROR | An **Intervention Menu** heading is present (the editor-discretion reframe of the revision guidance). Override: `<!-- override: scaffolding-checklist — <rationale> -->` in the body, for a run that deliberately keeps an author-facing checklist. |
...
| **W1 — author-directed prescription leak** | WARN (ERROR under `--strict`) | A curated lexicon of author-directed second-person prescriptive imperatives appears in the body ("you should rewrite", "you need to add", "you must cut", "rewrite the", "add a scene", "delete the"). In scaffolding mode the prescription belongs to the human editor; these address the author. Advisory because prose detection is heuristic and legitimate quotation exists; override `<!-- override: scaffolding-prescription — <rationale> -->`; `--strict` makes it a gate. |
