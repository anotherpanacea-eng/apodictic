...
**Status:** Increment 1 **built**; the **Editor ↔ author dual-output** increment **built** (see §Dual-output); **Per-pass editor scaffolding built** (see §Per-pass scaffolding); **Blind-spot ranking built** (see §Blind-spot ranking) — the last deferred editor-scaffolding increment. Roadmap: `ROADMAP.md` → Operators → Editor Scaffolding. Implementation: `plugins/apodictic/skills/core-editor/references/editor-scaffolding.md` (mode contract), `run-synthesis.md §Operator Mode: Editor Scaffolding` (synthesis hook), the intake-router `operator:editor` flip (gap → built), `scripts/editor_scaffolding.py`, `validate.sh editor-scaffolding` (+ `--dual` arm, `--per-pass` arm, and canonical `--check-all` gates), the `apodictic.blind_spot_ranking.v1` + `apodictic.finding.v1` (optional `salience`) schemas, and the worked examples `references/example-editorial-letter-scaffolded.md` (editor side, carries a ranked block) + `references/example-editorial-letter-dual-author.md` (author side) + `references/example-pass-scaffolded.md` (per-pass, carries a ranked block).
...
| **E2 — blind-spot section** | ERROR | A non-empty **What You Might Have Missed** section is present **in the body** (heading match, ≥1 content line). The scaffolding value-add cannot be silently dropped — and an appendix heading does not satisfy it. **Blind-spot ranking (opt-in):** if the section carries `apodictic:blind_spot_ranking` blocks, they must be in descending severity-vs-salience *gap* order — see §Blind-spot ranking. A section with no ranking blocks stays valid (unranked). |
...
| **P2 — blind-spot surfacing** | ERROR | A non-empty **`## What You Might Have Missed`** section **in the body** — the per-pass value-add (the findings at this pass's layer a confident read is most likely to under-weight). Reuses the E2 heading; an appendix heading does not satisfy it. Parallel to E2. **Blind-spot ranking (opt-in):** the same sub-check as E2 — if the section carries `apodictic:blind_spot_ranking` blocks they must be in descending severity-vs-salience gap order (§Blind-spot ranking); a section with no blocks stays valid (unranked). |
...
The blind-spot section's whole point is to surface what a confident first read under-weights — so the *order* of that section is itself a signal. Ordering by severity alone re-lists the obvious high-severity findings the editor already caught. Blind-spot ranking instead orders by the **gap** between a finding's structural severity and its surface **salience** (how noticeable it is on a confident first read), largest gap first — so a high-severity finding that is *easy to miss* (masked by strong prose) leads, while an obvious high-severity finding ranks lower.

**Salience signal (the finding contract).** `apodictic.finding.v1` gains an **optional** `salience` enum — `prominent` / `moderate` / `subtle` (`subtle` = easiest to miss). It is additive and backward-compatible: a finding without it stays valid (the subset checker ignores an absent optional field), and the model — never the validator — owns the judgment.

**Ranking block (self-contained, mirrors Severity Calibration).** The "What You Might Have Missed" section carries an ordered list of `apodictic.blind_spot_ranking.v1` blocks — **one per entry, in display order** — each `{finding_id, severity, salience}`. `severity` is duplicated into the block for self-containment and cross-checkability, exactly as `apodictic.severity_calibration.v1` duplicates `delivered`. The blocks are the machine-checkable statement of the section's order.

**The ranking sub-check (mechanical; extends E2 and P2, opt-in).** It fires only when the section carries ≥1 ranking block — a scaffolded letter/pass **without** blocks still passes E2/P2 unranked (backward-compatible; the canonical fixtures are not retro-broken). The validator never assigns salience or decides which findings are blind spots — it checks only the declared ranking's arithmetic + order:

- **R1 schema** — each block validates against `apodictic.blind_spot_ranking.v1` (closed-key; bound to `editor-scaffolding` in `schemas/_coverage.json`).
- **R2 order** — compute `gap = rank(severity) − rank(salience)` with `rank(Must-Fix)=3 / Should-Fix=2 / Could-Fix=1` and `rank(prominent)=3 / moderate=2 / subtle=1`. The blocks must be in **descending-gap** order, ties broken by **descending severity**, then stable. So a Must-Fix + subtle finding (gap 2) outranks an obvious Must-Fix + prominent finding (gap 0); mis-ordering is an ERROR.
- **R3 ledger cross-check (optional two-file)** — with a companion Findings Ledger (`editor-scaffolding … --ledger=<ledger.md>`), each `finding_id` must exist in the ledger and its declared `severity` must match the locked tier — a laundered severity fails. Without a ledger, the block's severity is checked for internal consistency (schema + gap order) only.

This keeps the firewall intact: the model owns severity *and* salience; the validator owns only the gap arithmetic and the sort order (no prose is read).

`validate.sh --check-all` exercises the ranked block on both canonical worked examples — the letter's E2 (`example-editorial-letter-scaffolded.md`) and the per-pass P2 (`example-pass-scaffolded.md`, under `--strict`) — each carrying two correctly gap-ordered entries.
...
**Blind-spot ranking (built):** the optional `salience` field on `apodictic.finding.v1`, the `apodictic.blind_spot_ranking.v1` block schema (bound to `editor-scaffolding`), the opt-in R1/R2/R3 ranking sub-check extending E2 and P2 (§Blind-spot ranking), the optional `--ledger=` cross-check arm, and the ranked blocks added to both canonical worked examples under `--check-all`. No new validator (the derived count is unchanged); the ranking sub-check is a mode/path on the existing `editor-scaffolding` validator.
