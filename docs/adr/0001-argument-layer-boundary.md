...
- **D6 — Completeness-contract scope.** The crosswalk covers the argumentation **code** layer: the **88** entries (52 Dialectical Clarity codes + 20 FM-A patterns + 8 scheme hints + 3 warrant-verdict values + 5 premise-plausibility flag types). Internal benchmark/structure scaffolding — the GT1–GT8 answer-key anchors, the `Argument_State` section numbers, the severity/blast-radius tiers — is **out of scope by design**, recorded here rather than silently omitted.
...
The whole purpose of this ADR is to cure taxonomy drift, so the validator must not become a fourth silent copy of the code list. Its authoritative 88-ID set is **derived at check time**: the verdict / premise-flag / FM-A slices are imported from the live owners in `argument_groundtruth.py` (`_GT7_CLASSES`, `_GT8_ROW_FLAG_TYPES`, `_FM_A_MAX`); the 52 Dialectical Clarity codes and 8 scheme hints — which have no machine-readable owner — are parsed structurally out of `dialectical-clarity.md`. Crosswalk membership must equal that derived union, and count tripwires assert the *enumerated* 52 and 20 (naming FM-A20), so stale registry self-counts cannot be propagated.
...
**R4B implementation note (2026-07-12).** The crosswalk advances to v0.2.0 without changing the 82-code membership: R2 relations and R3 move families live in a separate eight-row concept layer. The optional `apodictic.aif-core-export.v1` adapter is one-way and loss-aware over `Argument_State` v0.2.0. It emits only source-licensed I/RA/CA topology, never PA or a claim-ladder inference by guess, and cannot feed diagnoses back into the internal state. Its structural reference is the ARG-tech *AIF Specification*, Definition 1.1; a green gate proves export shape and source closure, not scholarly correctness or universal AIF interoperability. This amends how the Consequences work-list is read: R4B reviewed those `unmapped` rationales into present-tense conclusions rather than treating them as a quota for speculative mappings, so the rows that remain `unmapped` at v0.2.0 stay so because the scholarship does not warrant a mapping, not because they are pending work.

## Additive amendment — R5 ClaimLicense ↔ Toulmin (2026-07-12)

- **D8 — ClaimLicense is crosswalked by analogy, not identified with a Toulmin warrant.** Toulmin's model describes object-level roles inside a manuscript argument; SETEC Voiceprint's `ClaimLicense` is a meta-level governance object bounding how an audit result may be reported. The current whole-object→warrant mapping is `unmapped`: ClaimLicense has no explicit `entitlement_basis`, `inference_rule`, or equivalent bridge from named results to a named licensed conclusion. Its individual context, caveat, refusal, uncertainty, and reference fields carry `related` mappings only. SETEC's envelope-level `task_surface` is routing metadata duplicated into `claim_license.task_surface` under producer-enforced equality, not a Toulmin component. The reviewed field map and anti-equivalence cases live in [`docs/claim-license-toulmin-crosswalk.md`](../claim-license-toulmin-crosswalk.md).

**R5 consequence.** R5 realizes the final docs-only coherence step without changing `Argument_State`, SETEC, the normalized entrypoint, a version floor, any schema, any validator, or runtime behavior. The crosswalk is pinned as a documentation snapshot of setec-voiceprint v1.124.0 / envelope `schema_version: 1.0`; producer drift requires renewed documentary and scholarly review, not a new cross-repo runtime dependency.
