...
The sweep's other classes are deliberately out of scope for Increment 1 because they need per-validator semantics, not a structural grep: **crash-on-malformed** (a non-dict block payload reaching `obj.get()` — fixable per-validator with an `isinstance(obj, dict)` guard, the companion to this PR), **lexical-regex over/under-fire** (heuristic-tuning the specs already concede is fuzzy), and **override-parse** edge cases. A future increment could add a crash-resistance check (every `parse_*` guards non-dict before `.get()`), but that needs AST-level analysis rather than the line-level greps M1–M4 use.
