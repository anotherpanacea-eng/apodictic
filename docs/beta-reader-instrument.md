...
**Status:** Proposed (unbuilt). Roadmap: `ROADMAP.md` → [Horizon Capacities](../ROADMAP.md#horizon-capacities) Tier 1, item 3. Proposed implementation surface: a `revision-coach/references/beta-reader-instrument.md` mode, a `commands/reader-questions.md` command, `scripts/reader_instrument.py`, `validate.sh reader-instrument` (+ `--check-all` gate), and a worked example with a paired fixture ledger. Schema: `apodictic.reader_question.v1`.
