...
**The `revised` finding-state and how Increment 4 handles it.** The ladder below references the `locked → delivered → revised` lifecycle. `revised` has **two writers** (Increment 4a, `docs/revision-round-gate.md`): the `revision_round` gate folds the resolved-marker ids into `execution.finding_states[<id>] = "revised"` for **runner-governed** projects (so it stays fold-consistent), and the revision-round protocol writes the field **directly** for **non-governed** projects (`state-lifecycle.md`). The **canonical source** of "which findings were revised" is the `<!-- resolved: F-… -->` markers in the revision report (what both writers and `finding-trace` key on). So the dispatcher reads `finding_states.revised`, falling back to the markers. (Originally 4b shipped reading the markers directly while 4a — the gate writer — followed in its own cycle; both are now built.)
...
- `execution.finding_dispositions` — the declined/deferred overlay records (`docs/finding-dispositions.md`; pinned disposition markers as fallback)
...
3. `delivered`-but-not-`revised` Must-Fix → the highest-leverage open fix → offer `/coach` session planning scoped to it, or an execution-mode handoff. *(Step 3 is the loop's core; the `revised` discriminator comes from the source-of-truth rule above — available today, not gated on 4a.)* The selector skips ids with an active `declined` disposition and trigger-reviews active `deferred` ids (fired → re-enter at rank; unfired → skip) — the full rule lives in `revision-coach/SKILL.md` §Loop Dispatch.
4. All Must-Fix `revised`, Should-Fix open → next-tier coaching.
5. All findings `revised` or actively dispositioned, `control_questions.open == 0` → offer submission-readiness (`submission` node; declined Must-Fixes are listed in the verdict's caveat block, never absorbed).

**Stalled-revision off-ramp.** If a `delivered` Must-Fix has seen no progress across multiple sessions (Coaching Log) and still carries no resolved marker, step 3 must not re-offer the same `/coach` step indefinitely — surface "this finding has stalled: consider an execution-mode handoff, or set it aside" so the loop has a terminal move. Setting it aside records an **engine finding disposition** (`docs/finding-dispositions.md`; built with `disposition-check`): the pinned marker in the Coaching Log plus the `execution.finding_dispositions` record via the dual-writer path — `declined` or `deferred`+trigger, the writer's choice, never the dispatcher's. A later session then reads a deliberate pass mechanically instead of relying on a prose note.

The dispatcher proposes; the writer disposes. It replaces "remember whether to type `/coach` or `/ready`" with "the tool already knows where you are and what's worth doing next." This is the loop position and project identity that [Coaching Deepening](../ROADMAP.md#coaching-deepening) (Multi-Session Revision Arc Planning, Coaching History and Pattern Recognition) and [Collaborative Revision Coaching](../ROADMAP.md#collaborative-revision-coaching) build on but currently have no substrate for.
...
**Layer 4a — gated `revision_round` phase (the canonical `revised` writer). — Built (`docs/revision-round-gate.md`).** Adds a third phase to `execution-gates.v1.json` (`revision_round`, `allowed_next: ["run_synthesis"]`, a narrow `*_Revision_Report_*.md` artifact_key + attestations + a `ledger-check` *precondition* check), maps `_PHASE_FINDING_STATE["revision_round"] = "revised"` in `run_gate.py`, and — the delicate part — marks **only the resolved subset** of findings `revised` (the report's `<!-- resolved: F-… -->` ids, intersected with the ledger), unlike the existing phases that mark *all* ledger findings. The original plan was to *remove* the revision round's direct write; the spec review (Blocker 1) corrected that — most revision-stage projects have no `gate_events`, so the direct write is their only path and `check-state` never runs for them. **Final design: scope, don't remove** — the gate writes `revised` for runner-governed projects (so the `pointer == fold` invariant, `run_gate.py:662`, holds), and the direct write is retained for non-governed projects. A `finding-trace` *entry* check was also abandoned (it audits the post-condition the gate establishes, so it would block its own clear); `ledger-check` is the entry precondition and `finding-trace` is the post-hoc audit.
