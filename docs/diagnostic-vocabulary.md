...
**Enforcement scope.** The Vocabulary Guide is a *distinct named artifact* that only exists in facilitator mode (unlike the editorial letter, which is author-facing by default and only sometimes scaffolded). So the no-op is narrower here than in editor-scaffolding: a file that is **identifiably a Vocabulary Guide** — by a `*_Vocabulary_Guide_*.md` filename **or** a `# … Vocabulary Guide` H1 title — but is **missing** the `<!-- mode: diagnostic-vocabulary -->` marker is a **V0 ERROR** (otherwise V1–V4/W1 would be silently skipped — a false pass on the very artifact the validator exists to check). Only a genuinely **unrelated** file (no guide name/title, no marker) stays a no-op `0`. When the marker is present, the full contract is enforced. All section checks are **body-scoped** (everything before a first `Appendix A` heading, if any), so an appendix can't satisfy a required section.

| ID | Severity | Rule |
|---|---|---|
| **V0 — marker required on a guide** | ERROR | A file identifiable as a Vocabulary Guide (by `*_Vocabulary_Guide_*.md` name or a `# … Vocabulary Guide` title) is missing the `<!-- mode: diagnostic-vocabulary -->` marker. (Unrelated files without the marker remain a no-op pass.) |
