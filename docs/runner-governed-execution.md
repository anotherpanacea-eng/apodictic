...
**Enforcement.** A `passed` event is a **clearing pass** iff its `attested_items` cover its own `required_ids` — `attested_contract` if recorded, else the live-manifest fallback, else ∅ for a grandfathered `migrated` seed (formalized as `clearing_pass(e)` under Open exceptions). `gate-state` treats a `passed` that carries a contract but fails to cover it as a hard error (same standing as a missing required field), validates each event **against its own recorded contract only** (never the live manifest, so history is durable), and — defensively — the frontier fold and `open_exceptions` count only clearing passes, so even a malformed log cannot advertise an un-attested or mechanical-only gate as cleared. The "you may not cite an ID absent from the *current* manifest" check is a **writer-time** rule (see Attestation handshake), not a `gate-state` invariant — a static sidecar validator cannot know which event was just written.
...
```
required_ids(e)  = e.attested_contract            if present     // the event's OWN snapshot
                 = ∅  (grandfathered)              if e.migrated  // legacy seed, see Migration
                 = manifest.attested_ids(e.gate)   otherwise      // legacy/advisory fallback
clearing_pass(e) ≡ e.result == passed ∧ set(attested_items(e)) ⊇ set(required_ids(e))
                   // coverage uses the event's own recorded contract — never the live manifest
                   // for an event that carries one — so a later manifest edit can never
                   // retroactively un-clear history. Empty required_ids ⇒ vacuously clearing.
open_exceptions  = { gate : latest_event(gate) exists ∧ ¬ clearing_pass(latest_event(gate)) }
```

So a gate is open whenever its latest event is anything other than a clearing pass — that subsumes `blocked`, `pass-with-warn`, `skipped`, `deferred`, the non-clearing `mechanical-passed`, **and** a `passed` that fails coverage of *its own* `attested_contract`. (Review fix — Codex P1: the formal fold now uses `required_ids(e)` — the event's recorded contract — everywhere the prose already promised, instead of re-expanding to the live manifest, which would have re-introduced retroactive invalidation. Review fix — Codex P2: "not a clearing pass" also subsumes `mechanical-passed` / un-attested `passed`, which the earlier enumerated set silently excluded.)
...
- **Semantic (new `gate-state` Python validator).** `validate_obj` has **no conditional-required, no cross-field rules, no null/union types, and does not recurse into nested array-objects or object-map values** — so `gate-state` owns everything the subset checker cannot express, each with positive + negative fixtures (mirroring the `run_gate.py` / `apodictic_artifacts.py` self-test style):
  - **attestation coverage (Codex P1/P2):** a clearing `passed` must have `attested_items` whose **ID set ⊇ its own `attested_contract`** (deterministic inclusion, not string matching) — else it is not a clearing pass (hard error). `gate-state` validates each event **against its own recorded contract only** (`required_ids(e)`), so a later manifest rename/split/retire never retroactively invalidates history. An event with no `attested_contract` and not `migrated` falls back to the live manifest, **advisory-only** (never a hard error). The "recorded IDs ⊆ *current* manifest IDs" check is **not** a `gate-state` invariant — a static validator can't tell which event is new; it lives in the writer (`gate --attest`, above). A `migrated: true` seed is **grandfathered** (`required_ids = ∅`), so legacy history neither hard-fails nor needs attestation;
...
- **Writer rule (Increment-5 engine) — default: don't seed (review fix — Codex P1).** A legacy `passed` / `"attested"` entry has no `attested_items` / `attested_contract`, so seeding it as a clearing `passed` under the new rules would either fail coverage or (worse) advance the frontier on un-attested history. So the default migration **keeps the project at `state_version: 1` with its legacy `gates` map** until the next real gate event, which writes the first v2 event and flips `state_version: 2`. The reader fallback already resumes such projects, so nothing is lost; no fabricated clearing passes.
- **Optional seeding (history continuity) — grandfathered.** If a project wants its prior gate history in the log, seed one event per legacy `gates` entry, faithfully provenance-mapped (review fix — Codex P3): legacy `"attested"` → `{result: passed, provenance: attested}`; legacy `passed` / `pass-with-warn` / `blocked` → that result with `provenance: mechanical`. Each seed is flagged **`"migrated": true`** (`note: "migrated from legacy gates map"`, coarse `ts`), which **grandfathers** it: `required_ids = ∅`, so `gate-state` exempts it from attestation-coverage and freshness, and the fold treats a migrated `passed` as clearing. Migrated seeds thus preserve the frontier without fabricating attestations.
...
7. Migration: **default keeps legacy projects at `state_version: 1`** (legacy `gates` map + reader fallback) until the next real gate event; optional faithful seeding emits `migrated: true` grandfathered events. Reader keys off `state_version`.
