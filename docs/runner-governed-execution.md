...
| `attested_contract` | **required on a non-migrated clearing `passed`** | string[] | the required attested-ID set from the manifest **at write time** (`[]` for a gate with no attested items) — coverage is checked against *this* snapshot, so later manifest edits never retroactively invalidate the event. The writer always sets it; absence on a non-migrated `passed` is malformed. |
...
| `migrated` | optional | boolean | seeded from the legacy `gates` map (see Migration); **grandfathered** — `required_ids = ∅`, so exempt from attestation-coverage / freshness |
...
**Enforcement.** A `passed` event is a **clearing pass** iff it carries an `attested_contract` (or is a grandfathered `migrated` seed, `required_ids = ∅`) **and** its `attested_items` cover that `required_ids` (formalized as `clearing_pass(e)` under Open exceptions). `gate-state` treats a non-migrated `passed` that lacks `attested_contract`, or carries one it fails to cover, as a hard error (same standing as a missing required field), and validates each event **against its own recorded contract only** — never the live manifest — so history is durable and the gating fold never depends on the current manifest. Defensively, the frontier fold and `open_exceptions` count only clearing passes, so even a malformed log cannot advertise an un-attested or mechanical-only gate as cleared. The "you may not cite an ID absent from the *current* manifest" check is a **writer-time** rule (see Attestation handshake), not a `gate-state` invariant — a static sidecar validator cannot know which event was just written.
...
```
required_ids(e)  = ∅  (grandfathered)              if e.migrated        // legacy seed, see Migration
                 = e.attested_contract             otherwise           // REQUIRED on every non-migrated
                                                                       // v2 clearing pass (the writer
                                                                       // always sets it; [] for a gate
                                                                       // with no attested items)
clearing_pass(e) ≡ e.result == passed
                 ∧ (e.migrated ∨ e.attested_contract present)          // a non-migrated `passed` with no
                                                                       // contract is malformed → not clearing
                 ∧ set(attested_items(e)) ⊇ set(required_ids(e))
                   // coverage uses the event's OWN recorded contract (∅ if grandfathered). The live
                   // manifest is NEVER consulted in the fold, so a manifest edit can never
                   // retroactively un-clear a recorded pass.
open_exceptions  = { gate : latest_event(gate) exists ∧ ¬ clearing_pass(latest_event(gate)) }
```

So a gate is open whenever its latest event is anything other than a clearing pass — that subsumes `blocked`, `pass-with-warn`, `skipped`, `deferred`, the non-clearing `mechanical-passed`, **and** a `passed` that either lacks a contract (non-migrated) or fails to cover *its own* `attested_contract`. (Review fix — Codex P1: the fold uses `required_ids(e)` — the event's recorded contract, ∅ if grandfathered — and the live manifest never enters it; review fix — Codex P2: `attested_contract` is **required** on every non-migrated v2 clearing pass, so there is no live-manifest gating fallback that could flip a recorded pass to non-clearing after a manifest edit; review fix — earlier Codex P2: "not a clearing pass" also subsumes `mechanical-passed` / un-attested `passed`.)
...
- **Writer rule (Increment-5 engine) — seed a grandfathered baseline on upgrade (review fix — Codex P1).** The first time the engine upgrades a legacy sidecar to `state_version: 2` (atomically with the next real gate event), it **seeds one grandfathered event per legacy `gates` entry** so the v2 fold can reconstruct the prior frontier — otherwise, if that first real event is `mechanical-passed` / `blocked` / `pass-with-warn`, the fold would have no prior clearing event and would *lose* the legacy `execution.phase`. Seeds are faithfully provenance-mapped (review fix — earlier Codex P3): legacy `"attested"` → `{result: passed, provenance: attested}`; legacy `passed` / `pass-with-warn` / `blocked` → that result with `provenance: mechanical`. Each is flagged **`"migrated": true`** (`note: "migrated from legacy gates map"`, coarse `ts`), which **grandfathers** it: `required_ids = ∅`, so `gate-state` exempts it from attestation-coverage and freshness, and the fold treats a migrated `passed` as a clearing pass. So the baseline frontier is preserved **without fabricating attestations** (the seeds are explicitly `migrated`, never presented as real attested passes). After seeding, `gate_events` is self-contained and the legacy `gates` map may be dropped.
...
4. New **`gate-state`** validator (its own arm, or `run_gate.py --check-state <sidecar>`) owning the semantic invariants listed under *Enforcement* — attestation coverage against **each event's own recorded `attested_contract`** (`required_ids(e)`; *not* the live manifest, and `migrated` seeds grandfathered), check freshness + `artifact_digests` binding, conditional `reason`, provenance constraints, blind-spot presence, `finding_deltas` clearing-only, `checks[]` / `finding_deltas` inner shapes, the pointer/fold invariant, `open_exceptions` (= "latest event is not a clearing pass"), and the enum drift guard — each with positive + negative fixtures. Register in `--self-test-all` (17 → 18; bump the count strings + usage + docblocks in lockstep).
...
7. Migration: on upgrade to `state_version: 2`, **seed a grandfathered baseline** (one faithful `migrated: true` event per legacy `gates` entry) so the v2 fold preserves the prior frontier; reader keys off `state_version`. (Alternative zero-seed mode folds from the retained legacy map — see Migration.)
