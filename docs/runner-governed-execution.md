...
| `migrated` | optional | boolean | seeded from the legacy `gates` map (see Migration); legal **only as the contiguous upgrade-baseline prefix** (`gate-state` invariant). A validated grandfathered seed has `required_ids = ∅`, exempt from attestation-coverage / freshness |
...
- **Semantic (new `gate-state` Python validator).** `validate_obj` has **no conditional-required, no cross-field rules, no null/union types, and does not recurse into nested array-objects or object-map values** — so `gate-state` owns everything the subset checker cannot express, each with positive + negative fixtures (mirroring the `run_gate.py` / `apodictic_artifacts.py` self-test style):
  - **migration-prefix integrity (review fix — Codex P1):** `migrated: true` events are legal **only as a contiguous prefix** of `gate_events[]` (the baseline written during the `state_version 1 → 2` upgrade), with **at most one per gate**, in **manifest phase order**. A `migrated` event that follows any non-migrated event — or a second one for the same gate — is a **hard error**. This is what makes the grandfather exemption safe: it cannot be used as an ongoing clearing bypass (a hand-authored `migrated:true, result:passed` injected after real work fails this check, and the fold's `grandfathered(e)` denies it `required_ids = ∅` anyway). The migration baseline is trusted exactly as far as the legacy `gates` map it mirrors was — no further;
...
3. `run_gate.py`: **append** an event per run instead of overwriting `gates[<phase>]`; recompute the pointer (`phase`, `allowed_next`, `pending_gate`, `finding_states`) from the log; set `state_version: 2`. **Attestation handshake (Codex P1):** mechanical success on a gate with attested items writes the non-clearing **`mechanical-passed`** (with `artifact_digests`); a new `gate --attest <gate> <run_folder>` **re-runs the mechanical checks** and, only if still clean, records the clearing `passed` with `attested_items` + the **always-written** `attested_contract` (= the gate's *current* manifest IDs, `[]` for a no-attest gate) + the fresh `checks[]` + matching `artifact_digests`; `finding_deltas` ride the clearing event. **The live-manifest check is here in the writer (Codex P2):** `--attest` refuses to confirm any ID absent from the current manifest. A no-attest gate clears directly on mechanical success (still with `attested_contract: []`). Author `skipped` / `deferred` events (required `reason` + Audit-Invocation-Log blind-spot); add `gate --strict` (nonzero while `open_exceptions` is non-empty).
4. New **`gate-state`** validator (its own arm, or `run_gate.py --check-state <sidecar>`) owning the semantic invariants listed under *Enforcement* — the **migration-prefix integrity** check (migrated events only as a contiguous head, ≤1 per gate, manifest order), attestation coverage against **each event's own recorded `attested_contract`** (`required_ids(e)`; *not* the live manifest; grandfather only a *validated* prefix seed), check freshness + `artifact_digests` binding, conditional `reason`, provenance constraints, blind-spot presence, `finding_deltas` clearing-only, `checks[]` / `finding_deltas` inner shapes, the pointer/fold invariant, `open_exceptions` (= "latest event is not a clearing pass"), and the enum drift guard — each with positive + negative fixtures. Register in `--self-test-all` (17 → 18; bump the count strings + usage + docblocks in lockstep).
