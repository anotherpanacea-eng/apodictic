...
| `anti_thesis_target` / `anti_thesis_relation` *(optional, R2)* | §6 Objection 1's `Target`/`Relation` typing — flat scalar siblings (schema `pattern`/`enum`-checked), so `anti_thesis` stays a required string and W1's echo check is untouched |
...
| **B5 — dangling objection target** | ERROR | A typed `reviewer_objections` item's `target` names a `Cn` not declared in the spine's ladder (the A5/A8 machinery; skip-not-fail when no valid spine — the B3 precedent). Item **shape** (object form requires a non-whitespace `objection`; `target`/`relation` value-spaces; post-draft audit keys rejected) is B1-class, hand-rolled in `_reviewer_objection_errs` — the schema's `items` is untyped because the subset validator has no union support. *(R2)* |
