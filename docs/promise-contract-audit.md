...
**Status:** Proposed (unbuilt); **spec-reviewed BUILD-READY-WITH-FIXES** (2026-06-19 Codex-down pass — see [§Build-Readiness Review](#build-readiness-review-codex-down-independent-pass-2026-06-19) at the end). OD1 **resolved 2026-06-19** (operator: add a `CONTROLLING IDEA:` schema field). No open decisions remain — **buildable now** (spec→review→build→review→PR); the PR's *merge* is what waits on review (Codex, or a subagent stand-in while Codex is down). Roadmap: `ROADMAP.md` → [Horizon Capacities](../ROADMAP.md#horizon-capacities) Tier 1, item 4. Proposed implementation surface: a **core-editor workflow module** (homed like [Legal Risk Register](legal-risk-register.md) / Editor Scaffolding — *not* a craft audit), `core-editor/references/promise-contract.md`, an `apodictic.pitch_copy.v1` persisted input + `apodictic.finding.v1` findings with origin `PCF`, `scripts/promise_contract.py`, `validate.sh promise-contract`, and a worked example.
...
**OD1 — RESOLVED 2026-06-19 (operator): add a `CONTROLLING IDEA:` schema field.** `CONTROLLING IDEA` was not a colon-delimited contract field — `contract-template.md` exposed `READER PROMISE:` / `ENDING TYPE:` / `NON-NEGOTIABLES:` as schema fields, but the controlling idea lived only as a `## Controlling Idea` prose section (`**Statement:**` sub-line), and PCF1/PCF4 measure against `contract:CONTROLLING IDEA`. **Decision: (b)** — promote it to a first-class `CONTROLLING IDEA:` schema field (consistent with the other three; chosen over (a) parse-the-section despite (a)'s lower blast radius). **Build implications (the build can proceed now; the PR-merge is the only review gate):**
> - Edit `core-editor/references/contract-template.md` to add `CONTROLLING IDEA:` to the schema block alongside the other three fields; keep the `## Controlling Idea` prose section (it can restate / elaborate the field). This is a **behavioral change to a shared core artifact** (the contract the LLM produces every run) — additive, but verify nothing breaks on it:
>   - any validator/parser/generator that **enumerates** the contract schema fields (so a +1 field doesn't trip a count/parity check) — grep the validators + `release-generate.mjs` for contract-field references before building;
>   - the `contract:` ref resolver in `promise_contract.py` then treats `CONTROLLING IDEA` like the other fields (no special-case section parsing needed).
> - Settling this makes the spec turn-key — see the build checklist (a contract-template edit is now step 0).
...
0. `plugins/apodictic/skills/core-editor/references/contract-template.md` — **OD1:** add the `CONTROLLING IDEA:` schema field beside `READER PROMISE:` / `ENDING TYPE:` / `NON-NEGOTIABLES:` (keep the `## Controlling Idea` prose section). Behavioral change to a shared core artifact — first grep validators + `release-generate.mjs` for any contract-field enumerator that a +1 field could trip.
1. `plugins/apodictic/schemas/apodictic.pitch_copy.v1.schema.json` (new; `copy_type` enum + verbatim text; not mirrored).
2. `plugins/apodictic/skills/core-editor/references/promise-contract.md` (the module; `contract:CONTROLLING IDEA` now resolves to the schema field per OD1 — no section-parsing special case).
