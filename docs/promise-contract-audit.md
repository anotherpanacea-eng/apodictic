...
| **W1 — drafted-copy leak (firewall)** | WARN (ERROR under `--strict`) | A multi-sentence quoted block in the report is **not** a verbatim substring of the persisted pitch copy (→ authored replacement copy). Concrete substring check, not a vibe heuristic. Override is **snippet-keyed** (a leaked quote isn't attributable to one finding id): `<!-- override: drafted-copy <snippet of the leaked text> — <rationale> -->` silences only the leak its snippet matches. |
