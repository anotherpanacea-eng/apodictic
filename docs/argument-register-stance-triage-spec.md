...
*Status: Built — shared schema, AT5/GN reference layer, routing/synthesis integration, and the stance-calibration shape gate are implemented; E1-E7 are green, including two independent, machine-conformant E7 blind records.*
...
7. **The shape gate proves recorded calibration, not undeclared behavior.** A native Could-Fix finding with no stance fields is indistinguishable from a silent demotion unless the mechanism family itself supplies a complete reverse inventory. The validator therefore guarantees that recorded earned verdicts demote honestly, that an actual `register-floor`/`stance-demotion` is limited to the enumerated mechanism families, and that recorded blocks/joins are coherent; it does not prove that every eligible finding was declared or that a producer labeled a mechanism truthfully. Auditor protocol and E7 cover that boundary in this increment.
...
| E7 | Good-faith hybrid fixture: ~1,500 words of genuine, fertile lens play landing on ONE hard, high-consequence prescription | `register=generative` (confirmed) | Journey findings calibrated per the register; the prescription cash-out span assessed at full asserted burden with its findings ineligible for demotion (per-span backstop, Architecture Decision 4); no document-wide re-flood of the journey. | **PASS** — two fresh independent strict blind runs used canonical `CO#` joins and exact `UNWARRANTED` verdict tokens. Both separated the successful generative journey from the asserted policy landing and recorded would-be earned demotions as blocked at prescriptive joins; both vendored ledger/state pairs pass `stance-calibration` (A: 2 MF / 5 SF / 0 CF; B: 3 MF / 6 SF / 0 CF). |
...
Seven blind Opus auditors across three historical waves; none saw this spec, the expected outcomes, or each other. Machinery-arm auditors received a blinded addendum (this spec's Parts 1–2 stripped of examples and expectations). Fixture texts, the addendum, and the run summary are vendored in `evals/register-stance-pilot/`; the full historical ledgers are not present in this checkout. *(Historical pilot severity labels use the run-harness-injected 4-band scale — see §Validation for the canonical remap. The vendored addendum was refreshed before the strict E7 rerun and now states the built Triage-time/Could-Fix/precedence contract directly. The two fresh E7 auditors were blind to these historical records and to the spec.)*
...
### Carry-forwards (post-build, tracked)

1. **Letter-synthesis clustering guidance** for mixed-register / high-count ledgers (A4 produced 33 findings; the Argument-DE letter needs cluster-first guidance).
2. **Abbreviated triage under an active document-wide gate**: both gated runs spent effort fully documenting verdicts that could not demote; the reference should permit a compact record form (stance + verdict + one-line rationale) when no demotion is possible.
