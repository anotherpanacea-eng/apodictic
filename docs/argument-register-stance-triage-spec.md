...
**Stakes-gate interaction:** under an intake-declared high-stakes gate the triage still runs and the stance is still recorded — the record is diagnostic gold for the writer — but no verdict demotes severity. An earned S3 in draft testimony gets: `stance: S3, stance_verdict: earned, severity: unchanged, note: "high-stakes form; consider whether this register survives an adversarial reader."` Findings located at prescriptive cash-out spans are likewise ineligible (Architecture Decision 4), regardless of register or verdict.
...
2. **Writer-declared register means the engine trusts the writer.** Right trade for a development editor whose client is the writer; it forecloses using the engine as a neutral detector of bad-faith argument (Architecture Decision 8). GN2 and the per-span cash-out backstop partially compensate — but only partially.
...
6. **Content-resolved consequence is a heuristic and always will be.** The mechanical backstop (prescriptive cash-outs ineligible by type) catches the load-bearing case; a piece whose consequence is real but implicit (self-help with no imperative sentence) can still slip the heuristic. The intake-declared gate exists for the forms where slipping is unacceptable.
...
1. **E7 hybrid fixture**: specified in §Validation; must run and pass before the reference-file merge.
