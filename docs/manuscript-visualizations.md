...
| ID | Severity | Rule |
|---|---|---|
| **E1 — manifest schema** | ERROR | The `apodictic.viz_manifest.v1` block parses and satisfies the field schema (scene/finding arrays well-formed; **no visual-style fields present** — their presence is itself a failure, since style is the renderer's, not the run's). |
