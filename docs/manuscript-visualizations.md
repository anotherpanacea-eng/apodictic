...
Charts 4–7 are the Manuscript-Visualization Completion increment. Two ship today — chart 7-nonfiction (the claim ladder, render-only) and chart 5 (the co-presence network, over the `apodictic.scene_roster.v1` producer); the remaining three (4/6/7-fiction) are producer-gated.
...
Charts 1–3 are Increment 1 because their data is already fully structured; chart 7-nonfiction (the claim ladder) ships render-only because `argument_spine.v1` + `support_plan.v1` are already structured; chart 5 (co-presence) ships with its dedicated `apodictic.scene_roster.v1` producer; 4/6/7-fiction wait on their upstream producers, in the Harness Contracts v2 spirit. Until a producer lands, a present `scene_functions`/`reveal_points` array is a hard X8 failure (no producer to byte-check it against) — a present `co_presence` array is now legitimate iff it byte-checks against `scene_roster.v1`.
...
- Charts 4/6 (reveal/tension timeline, scene-function heatmap) and chart 7-fiction (beat-map-against-spine) — each gated on a **producer** increment (`apodictic.tension_point.v1` / `apodictic.scene_function.v1` / `apodictic.story_spine.v1`) that makes its upstream artifact machine-readable, plus (for charts 4/6) a validated `evidence_ref → scene_id` resolution for scene-precise placement.
