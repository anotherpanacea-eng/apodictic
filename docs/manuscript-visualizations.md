...
**Status:** **Increment 1 built** (charts 1–3: pacing/word-count curve, POV time-share, finding-severity-by-chapter). Roadmap: `ROADMAP.md` → [Horizon Capacities](../ROADMAP.md#horizon-capacities) Tier 1, item 1. Implementation surface: the `viz` reference module `core-editor/references/manuscript-visualizations.md`, the `scripts/viz_manifest.py` extractor/validator, `validate.sh manuscript-viz` (+ `--check-all` gate), the new single-file SVG renderer (`viz_manifest.py render`), and a worked example (`example-structure-map-manifest.md` paired with `example-timeline.md` + `example-findings-ledger.md`). **Future increments:** the character co-presence network, scene-function heatmap, reveal-economy timeline, and beat-map-against-spine charts (each gated on its upstream artifact becoming machine-readable — see §The chart set).
