...
| **O5 — letter prose fidelity** (Inc 2) | ERROR | Stripping the Obsidian letter's additions (the appended ` ^id` block ids + the converted `[[…]]` wikilinks) yields the **same** prose as stripping the gated crosslinked letter's CriticMarkup back-link spans — i.e. the letter projection mutated no editorial prose (the X4 analog). |
