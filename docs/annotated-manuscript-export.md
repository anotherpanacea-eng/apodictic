...
**Open questions for spec-review:** (Q1) does Obsidian resolve `[[copy#Chapter 9]]` against `# Chapter 9[^F-RR-01]` (heading-slug strips the footnote ref)? — decides heading-level vs file-level reverse-nav; (Q2) where exactly does the `^id` block id attach when the `<!-- finding -->` marker is mid-paragraph vs end-of-line (Obsidian block-id placement rules)? (Q3) confirm the in-place span→wikilink replacement keeps O5's strip-equivalence exact; (Q4) the paired copy+letter fixture under `obsidian/`, gate-valid under O1–O5.
