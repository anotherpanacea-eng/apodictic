...
**Status:** Planned (unbuilt) — promoted to the Board 2026-06-16 from Horizon Capacities Tier 1, item 2. Roadmap: `ROADMAP.md` → [§Annotated-Manuscript Deliverable](../ROADMAP.md#annotated-manuscript-deliverable). Proposed implementation surface: a reference module under `core-editor/references/annotated-manuscript.md`, a `scripts/annotation_manifest.py` extractor/validator, `validate.sh annotated-manuscript`, a CriticMarkup renderer, and a worked example under `core-editor/references/`.
