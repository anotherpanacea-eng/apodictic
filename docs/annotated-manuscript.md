...
- **X3 — positional pairing, no dangling** — the check is **positional** *and enumerates every span*, not a global set/count. It iterates **every** `{>> … <<}` span (the shared span regex, not just back-link-shaped ones) — because X4 deletes *all* spans before comparing, an authored/malformed span would otherwise ride through (build-review caught the *swap* variant; Codex caught the *authored-span* variant). So: (a) the crosslinked letter's `finding:` markers, in order, **equal** the letter's (a span injected *inside* a `<!-- finding: … -->` marker, which reverses cleanly under X4, is caught here); (b) **every** CriticMarkup span must be a well-formed back-link (an authored / un-manifested span fails) sitting **immediately after** a marker (start == that marker's `-->` end — an orphan / in-prose / doubled span fails), carrying **that marker's** `finding_id` (a swapped/mis-attributed back-link fails), and resolving to a manifest annotation (a phantom fails); (c) every marker of an annotated finding has its correctly-placed back-link (no missing reverse link).
