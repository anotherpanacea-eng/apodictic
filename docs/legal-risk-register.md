...
**Status:** Increment 1 **built**; **detection layer built** — per-class detection guidance + a standard escalation-trigger taxonomy in `core-editor/references/legal-risk-register.md` §Detection guidance / §Escalation-trigger taxonomy, synthesized from the cross-model research in [`docs/legal-risk-detection-level-setting.md`](legal-risk-detection-level-setting.md); **content-detection auto-recommend built** — the **prompt/router-only** design (model-side detection in the synthesis/routing prose, deliberately **not** a mechanical validator arm): the register is auto-offered for memoir / autofiction / nonfiction portraying identifiable real people even without an explicit `constraint:risk` flag (`core-editor/references/run-synthesis.md` §Content-detection auto-recommend, `intake-router-runtime.md` §3/§6 Table B, `pass-dependencies.md` §4a). Roadmap: `ROADMAP.md` → Workflows → Legal Risk Register. Home: **core-editor** (manuscript-content analysis), reference `core-editor/references/legal-risk-register.md`. Artifact: `[Project]_Legal_Risk_Register_[runlabel].md`. Validator: `validate.sh legal-risk`.
<!-- built-when: plugins/apodictic/skills/core-editor/references/run-synthesis.md contains "Content-detection auto-recommend" -->
<!-- Prose-only increment (no new code symbol): the built-when marker anchors on the synthesis-prose subsection that carries the behavior, per docs/qol-status-drift-lint.md §Marker syntax (Form 2 / `contains`). -->
...
- **Content-detection auto-recommend (prompt/router-only).** The register is now auto-offered for memoir / autofiction / nonfiction portraying identifiable real people *without* the explicit `constraint:risk` flag. Per the maintainer's call, this is **model-side detection in the routing/synthesis prose — not a mechanical validator arm** (no new validator, no validator-count change): the model reads the manuscript for the memoir / real-people signals (a mode signal + a real-people content signal, per §Detection guidance) and **offers** the register — same offer-then-attach discipline as the explicit-flag path, because the not-a-lawyer framing warrants an explicit confirm. Prose homes: `core-editor/references/run-synthesis.md` §Content-detection auto-recommend, `core-editor/references/legal-risk-register.md` §Auto-recommend, `intake-router-runtime.md` §3/§6 Table B, `pass-dependencies.md` §4a. Firewall intact: it flags/offers; it never adjudicates and never auto-produces the register without confirm.

**Future increments:**
- *(None currently planned. The content-detection auto-recommend above — the last named future increment — is now built.)*
