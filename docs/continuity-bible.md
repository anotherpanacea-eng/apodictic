...
### The State axis (contradiction axis — Increment 2, 2026-07-05)

The ledger table carries a **`State` column** — the contradiction *fact-state* axis, orthogonal to the editorial Must/Should/Could severity scale (a self-contradiction is a fact-state, not a defect — it must not be forced onto the severity scale). It rides the existing plain-markdown ledger; **no new schema**. Values (the shared `scripts/contradiction_state.py` helper, imported by both bibles):

- `conflicting` — a live, un-explained collision (the default for any written row).
- `apparent` — a collision the author marks intentional via a `<!-- override: bible-contradiction CF-NN/CF-MM — <in-world rationale> -->` marker (order-insensitive; distinct from `bible-rederive`, the C4 escape hatch).
- `consistent` — no collision; **never written as a row**.

The state is **mechanically derived** from the literal collision + the override presence (`derive_state` — no collision → `consistent`; collision + matching override → `apparent`; else → `conflicting`), never model-judged. The `X1` firewall (a new arm on the existing `continuity-bible` validator) proves: the `State` token is a valid enum AND matches the derivation (an author-asserted state that disagrees with the override reality FAILs); the register carries no editorial-severity token and no `apodictic:finding` block (the Content-Advisory A3 precedent). Every ledger row must also pair **at least two distinct `CF-NN` ids that each resolve to a real, well-formed `canon_fact` block** — a fabricated or single id FAILs `X1` referential integrity (a row must cite real, distinct facts before its state means anything). A ledger that carries data rows but **no `State` column** is the pre-axis form: accepted (the column is additive), but it WARNs loudly (ERROR under `--strict`) to nudge the author to add the axis. `conflicting` rows are surfaced for the editorial letter to cite in prose (Stage A wiring — the Legal-Risk / Setup–Payoff precedent), so an unresolved self-contradiction reaches the author's revision plan; an `apparent` row is intentional and is not cited. Anchored on ConStory-Bench (arXiv:2603.05890) + DOME (arXiv:2412.13575) — the taxonomy/benchmark only; detection stays mechanical.
