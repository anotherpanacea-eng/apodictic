...
The state is **mechanically derived** from the literal collision + the override presence (`derive_state` — no collision → `consistent`; collision + matching override → `apparent`; else → `conflicting`), never model-judged. The `X1` firewall (a new arm on the existing `continuity-bible` validator) proves: the `State` token is a valid enum AND matches the derivation (an author-asserted state that disagrees with the override reality FAILs); the register carries no editorial-severity token and no `apodictic:finding` block (the Content-Advisory A3 precedent). `conflicting` rows are surfaced for the editorial letter to cite in prose (Stage A wiring — the Legal-Risk / Setup–Payoff precedent), so an unresolved self-contradiction reaches the author's revision plan; an `apparent` row is intentional and is not cited. Anchored on ConStory-Bench (arXiv:2603.05890) + DOME (arXiv:2412.13575) — the taxonomy/benchmark only; detection stays mechanical.
