...
- **Severity is anchored to the `target` persona.** A finding Must-Fix for the target audience stays Must-Fix; a non-target persona for whom it reads `engaged` does **not** downgrade it. Mechanically: a `divergence.v1` block whose **optional `asserted_severity`** is *lower* than the anchored finding's **locked** severity in the Ledger fails `D3`. (The locked side is retrievable — `honesty_check.py`'s locked-findings parse reads `apodictic.finding.v1` severities by ID, exactly as `softness-check` does. The block side only exists when the map chooses to assert a severity, which is the only case where downgrading is even possible.)
...
| **D3 — target-severity anchoring** | ERROR | Exactly one persona is `target: true`; and no `divergence.v1` block's optional `asserted_severity` is **lower** than its anchored finding's locked Ledger severity. Segmentation may not downgrade the target-audience verdict. |
...
- *Why severity anchoring needs an explicit field* — `D3` can only fire on something the block actually asserts, so downgrade-detection lives on an optional `asserted_severity` compared against the locked Ledger severity; without that field there was nothing to read and the anti-persona-shopping claim was empty.
