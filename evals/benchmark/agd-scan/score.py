...
def validate_manifest_body(manifest, fixture, vendor, rep):
    ...
    for key, want in (("fixture_id", fixture), ("vendor", vendor), ("rep", rep)):
        ...
        # Type-exact, not merely ==: Python equality would accept rep true/1.0
        # for rep 1 (bool is an int subclass; float compares equal), violating
        # the pinned integer schema.
...
def render_report(results, gate):
    ...
    L.append("**Provenance.** Producer **%s** (prompt fingerprint "
             ...
             "and the transports + the full acquisition procedure — including the "
             "documented host-subagent-adapter procedure that produced the fable "
             "cells (not a re-runnable shell one-liner) — in the README and "
...
def _selftest():
    ...
    # type-exact identity: Python equality alone would accept these (bool is an
    # int subclass; 1.0 == 1) — the pinned schema says INTEGER rep
    check("rep true rejected (bool is not the pinned int)",
          validate_manifest_body(dict(good, rep=True), GATE_FIXTURE, "fable", 1) is not None)
    check("rep 1.0 rejected (float is not the pinned int)",
          validate_manifest_body(dict(good, rep=1.0), GATE_FIXTURE, "fable", 1) is not None)
