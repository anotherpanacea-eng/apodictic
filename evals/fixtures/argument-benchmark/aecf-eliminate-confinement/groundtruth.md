# Ground Truth: aecf-eliminate-confinement

## Provenance

- **Fixture slug:** aecf-eliminate-confinement
- **Bucket:** 2 policy brief · **Cluster:** C (criminal justice / probation) · stance: reform advocacy
- **Source class:** third-party published — **referenced (text NOT stored; copyright)**
- **Work:** Annie E. Casey Foundation, "Eliminate Confinement as a Response to Probation Rule Violations."
- **Pinned source:** https://www.aecf.org/resources/eliminate-confinement-as-a-response-to-probation-rule-violations (record retrieval date + content hash on first authoritative run).
- **Quotation policy:** paraphrase only.
- **Ground-truth authority:** Joshua A. Miller (editor), diagnosis registered 2026-05-30, before any run.
- **Scope:** GT1–GT3 authoritative. **GT4–GT7 PROVISIONAL.**

## GT1 — Main claim *(Q1)*

- **Expected C0:** "Young people should not be confined as a response to noncriminal (technical) probation rule violations."
- **Acceptable paraphrase band:** any recovery of *end confinement for technical/noncriminal youth probation violations* is a hit. "Juvenile justice should be reformed" is too broad (a partial).

## GT2 — Failure locus *(Q2)*

- **Main structural problem (authority):** harm, adolescent-development, racial-disparity, and efficacy arguments are **compressed into one policy ask** — four distinct support lines, none clearly load-bearing.
- **Primary failure layer:** SUPPORT / CLAIM-organization — the reader can't tell which argument carries the ask.
- **Provisional codes:** CL2 (subclaim organization) / SM3 (convergent pile, one direction); FM-A2 (The Evidence Pile).

## GT3 — Strongest real objection *(Q3)*

- **Objection zone (authority):** what to do with **repeated, high-risk noncriminal violations** short of confinement — the hard-case the categorical ask must address.
- **Provisional codes:** OB3 + FM-A18 (Implementation Blindspot — the alternative for the hard case); AT3 → BP5 (alternatives) adjacency.

## GT4–GT7 — *(PROVISIONAL)*

- **GT4 audience:** MIXED (policymakers + practitioners) · SYMPATHETIC→MIXED · HIGH.
- **GT5 red-team:** load-bearing = the high-risk-recidivism hard case + which support line actually bears the ask. Decoy = disputing one statistic.
- **GT6 repair order:** first target = **sequence/prioritize the four support lines** and address the hard-case alternative, before adding more disparity data.
- **GT7 Distinguish:** SOUND — a competent advocacy brief; soft spots are compression + unaddressed hard case (Should-Fix). **Calibration trap:** over-pathologizing a clear advocacy ask, or missing the categorical claim's edge-case gap.

## Notes

Pairs with `ppi-one-size-fits-none` (same domain, different weakness — causal attribution vs. compression). Cluster C tests whether the engine distinguishes *two reform briefs on the same topic* by their structure, not their shared politics.
