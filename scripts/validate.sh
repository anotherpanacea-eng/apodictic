...
PY
    ...
    if [ -d "$CA_BASE/example-annotated-manuscript" ] && command -v python3 >/dev/null 2>&1; then
      ...
      for CA_PXE_F in "$CA_PXE"/pdf/*.pdf; do
        if [ -f "$CA_PXE_F" ]; then
          CA_PXE_N=$((CA_PXE_N + 1))
          cmp -s "$CA_PXE_F" "$CA_PXE_SRC/pdf/$(basename "$CA_PXE_F")" || CA_PXE_OK=0
        else
          CA_PXE_OK=0
        fi
      done
      ...
      if [ "$CA_PXE_OK" -eq 1 ]; then
        echo "pdf-export (temp copy): PASS"
      else
...
EOF
  ...
  cost-floor)
    # Record-integrity gate for a user-DECLARED budget cap that caps the run BELOW the token-fit
    # floor (run-core.md §Cost-floor override (budget cap)). Python-primary via config_checks.py —
    # the check composes override-marker scanning (the override_marker SSoT) + the cost_floor_override
    # token grammar + Q1-Q5 trigger recomputation, so per the fleet NEW-VALIDATOR convention it
    # degrades to an advisory WARN without python3, NOT a second bash implementation (a duplicate
    # drifts). Reports + gates record integrity (CF1-CF4); it does NOT select the mode — the
    # orchestrator owns selection, like the quality-risk-triggers sibling.
    CF_DIR=$(cd "$(dirname "$0")" && pwd)
    CF_HELPER="$CF_DIR/config_checks.py"
    if [ "${1:-}" = "--self-test" ]; then
      if command -v python3 >/dev/null 2>&1 && [ -f "$CF_HELPER" ]; then python3 "$CF_HELPER" --self-test cost-floor; exit $?; fi
      echo "Self-test: PASS (degraded — python3 unavailable; cost-floor is advisory without it)"; exit 0
    fi
    if command -v python3 >/dev/null 2>&1 && [ -f "$CF_HELPER" ]; then
      if [ $# -lt 1 ]; then echo "Usage: $0 cost-floor <contract_or_run_metadata_file> [<preflight_packet>] [<meta_json>] [--strict] | --self-test"; exit 2; fi
      python3 "$CF_HELPER" cost-floor "$@"; exit $?
    fi
    echo "WARN: python3 unavailable — cost-floor skipped; check inline that a marker and a cost_floor_override token both exist and agree on mode (neither orphaned), a single-agent cap declares context_tier: large and has estimated single-agent load < 600K, a sequential/hybrid cap below the token-fit floor is disclosure-acknowledged, and every fired Q-trigger above the cap carries its own quality-risk-Q[n] override marker. See run-core.md §Cost-floor override (budget cap)."
    exit 0
    ;;
