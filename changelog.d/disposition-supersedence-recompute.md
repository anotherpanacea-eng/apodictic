...
and is surfaced as the new **DP2.6** WARN (ERROR under `--strict`). Self-tests grow 35 → 44
(keystone exploit repro, archived-evidence exemption, evidence-not-leaked negative, DP2.5
no-longer-suppressed); `--check-all` gains hostile arm 4 (fabricated supersedence must fail
closed). Spec: `docs/disposition-supersedence-recompute.md`.
