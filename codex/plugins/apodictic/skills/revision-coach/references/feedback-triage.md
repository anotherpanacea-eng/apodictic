...
*Reference file for the APODICTIC Revision Coach. Loaded by `/triage-feedback` (and `apodictic-coach` → Feedback Triage mode). For writers returning with external feedback. Spec + validator: `docs/feedback-triage.md`, `scripts/validate.sh feedback-triage`. Worked example: `../../core-editor/references/example-feedback-triage.md`.*
...
The coach **structures and prioritizes**; it does **not** validate claims by re-diagnosing. Validating "does the midpoint actually sag?" is a **targeted Core Editor pass**, not a coaching act. When a claim needs checking, run or recommend the specific pass (`apodictic-diagnose` targeted) and read its result back into the triage. A claim the diagnosis confirms is a **candidate finding** — it enters the Findings Ledger, where its severity is governed normally. This keeps Guidance Without Specification intact: the coach never invents the diagnosis behind a piece of feedback.
...
6. **Gate & hand off.** Run `scripts/validate.sh feedback-triage <run_folder>` (`--strict` for CI). Resolve ERRORs (broken contract, dangling/self conflict reference); review W1/W2. Hand the **act-now** set to Session Planning (`apodictic-coach`) as the next session's targets.
...
- **Next session targets** — the act-now items, sequenced, ready for `apodictic-coach`.
