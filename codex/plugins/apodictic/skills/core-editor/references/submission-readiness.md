...
**Status:** v1.1
**Entry point:** `apodictic-ready` command or `full_draft + submit` route via `apodictic-start`
...
- `apodictic-start` routes here when artifact = `full_draft`, goal = `submit`, no `constraint:time`
...
- When you've revised, run `apodictic-ready` again or a targeted `apodictic-diagnose` on the specific concerns.
...
### Command shortcut
`apodictic-ready` invokes this workflow directly, bypassing the `apodictic-start` router's Q1-Q3 sequence. The command pre-fills artifact = `full_draft` and goal = `submit`, then runs the abbreviated intake (publication path, query materials, execution mode).
...
- Run `apodictic-ready` again for a full re-assessment
- Run Submission Triage for a quick re-check
- Run a targeted `apodictic-diagnose` on specific concerns from the non-negotiables
