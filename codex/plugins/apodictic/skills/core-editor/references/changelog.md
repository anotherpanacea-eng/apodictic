...
- **Wave 2 B3a — Bold-paragraph subhead detection.** C1 subhead-cluster count extended to detect both level-3 markdown subheads AND bold-paragraph subheads (`**Keep**`, `**Cut / Release**`, etc.) as cluster delimiters. Both forms are equivalent semantically; the validator now treats them as equivalent. Self-test adds `c1b_bold_subhead_clusters` (mirrors canonical F1 Opus 4.7 fixture pattern: 13 sub-bullets across 3 bold-paragraph subheads → 3 cluster count → PASS).
...
The Phase 4 Wave 3 eval-coverage report (`docs/review-log/2026-04-25_phase-4-eval-coverage.md`) documented four calibration findings against the four canonical fixtures (F1 Opus + Codex, the F2 story, TAY argument-DE). All four FAILs were calibration artifacts, not contract violations. v1.8.0 calibrates the validator's parsing logic to close them.
...
Total prose change: ~196 lines added across framework files. No validators added (existing `audit-signal-propagation` from Phase 4 covers the table without modification). No fixture changes (CR-3 closure is documentation; CR-8 operationalization is documentation; the validator behavior on F1 Opus / Codex / TAY / the F2 story fixtures from Phase 4 Wave 3 is unchanged — §4e refines per-audit defaults but the canonical rule still applies for un-enumerated audits).
...
- Pass 10 Timeline cross-fixture eval coverage (F1 / the F2 story / TAY): the Wave 1 release ships the schema + validators + synthesis integration; live-fixture re-runs are downstream eval work.
...
- **`docs/review-log/2026-04-25_phase-4-eval-coverage.md`** — new review-log entry documenting validator runs against the four canonical Stage 1 fixtures (F1 Opus, F1 Codex, the F2 story, TAY).
...
- `apodictic-start` presents context-aware summary from sidecar: session count, root cause count, revision progress, next action
...
- **`apodictic-ready` command** — direct entry point bypassing the `apodictic-start` router's Q1-Q3 sequence.
...
- `apodictic-start` routes here when artifact = `full_draft` and goal = `submit` without time constraint.
...
- `apodictic-start` now runs a mode-aware resume gate before router Q1. If `Diagnostic_State.md` indicates execution mode, it presents: Check the fix / Keep working / Start fresh.
...
- `apodictic-develop-edit` is now defined as a `apodictic-start` shortcut with prefilled `artifact=full_draft`, `goal=repair`.
- `apodictic-pre-writing` is now defined as a `apodictic-start` shortcut with prefilled `artifact=idea`, `goal=draft`.
- `apodictic-diagnose` is now defined as targeted resolver routing (`goal=repair`, concern-required).
...
- **`apodictic-develop-edit` workflow order** corrected: audit integration now happens after core passes and before synthesis (step 4), matching `run-core.md` §Audit Integration Point. Previously synthesis preceded audit evaluation.
...
- **Intake router** (`intake-router.md`): added "Run a focused audit on a specific concern" option (E) for full_draft artifact, with corresponding route map entry. Previously no audit route was reachable for complete drafts via `apodictic-start`.
...
- New artifact in `run-core.md` §Audit Integration Point: `[Project]_Audit_Invocation_Log_[runlabel].md`. Tracks every audit considered during a run with source (universal/contract/finding-driven), status (run/skipped/deferred), and reason. Referenced in `apodictic-develop-edit` step 4.
...
### Deferred
- **P2 #5 (Token overhead / router split):** Splitting `intake-router.md` into a short runtime decision table and a separate design document would reduce early-turn context consumption. Deferred to future version — requires updating all references and the `apodictic-start` command.
...
- **Patched `apodictic-start` command:** No longer preloads all three companion skills. Loads `core-editor` only; loads target skill after route decision.
- **Patched `apodictic-pre-writing` command:** No longer preloads `core-editor` and `plot-architecture` at session start. Loads `pre-writing-pathway` only; loads `plot-architecture` at Phase 4; loads core only on re-entry.
- **Patched `apodictic-develop-edit` command:** Added lazy loading instructions for `references/run-core.md`, genre modules, `references/output-policy.md`, and `references/run-full.md`. Fixed cap drift: "surgery list (max 25 items)" → "revision checklist (max 10 items)" to match current output policy.
- **Version bumped to 4.14.**

### Context Load Impact
- `apodictic-start` → loads ~253 lines (was ~1,660 + two companion skills)
- `apodictic-develop-edit` → loads ~253 + ~450 (run-core) = ~700 lines (was ~1,660)
...
### Added
- **Intake Router v1** — single entry point (`apodictic-start`) that routes users to the right workflow in 2–3 questions
...
- **Router integration in Core DE intake:** When user arrives via `apodictic-start`, router output (artifact, goal, constraint flags) skips redundant intake questions.
- New `apodictic-start` command (9 commands total, up from 8).
...
### Notes
- Existing commands (`apodictic-develop-edit`, `apodictic-pre-writing`, `apodictic-diagnose`, etc.) remain functional as direct-access shortcuts. The router is the recommended entry point for new users.
...
### Added
- **Pre-Writing Pathway** — new skill and `apodictic-pre-writing` command guiding writers from idea to draftable structure without a manuscript
