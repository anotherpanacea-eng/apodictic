...
**Audit count: 33 → 34 (craft 15 → 16).** Specialized audits 30 → 31. Source of truth is `release-registry.json` (Craft category + `counts.availableAudits` / `counts.specializedAudits`); derived count strings regenerate to 34/16 across the READMEs, plugin/marketplace manifests, and the `apodictic-audit` command list.
...
- `plugins/apodictic/commands/audit.md` — `apodictic-audit` Craft list entry.
