...
> Do not load this file for `apodictic-start`.
...
Replace the current command-selection model (`apodictic-develop-edit`, `apodictic-new-project`, `apodictic-pre-writing`, etc.) with a single entry point that routes users to the right workflow in 2–3 questions. Users should never need to know framework internals.
...
A single command — `apodictic-start` or simply beginning a conversation with the plugin — triggers the router. All existing commands remain functional as direct-access shortcuts for users who already know what they want.
...
|----------------|-------------------|-------------------|
| `apodictic-start` | The router itself | Yes (primary entry) |
| `apodictic-develop-edit` | full_draft + repair | Yes |
| `apodictic-new-project` | Initializes project workspace, then runs router | Yes (infrastructure) |
| `apodictic-pre-writing` | idea + draft | Yes |
| `apodictic-diagnose` | Any artifact + repair (targeted) | Yes |
| `apodictic-revision-plan` | Post-diagnostic; not an intake question | Yes |
| `apodictic-plot-coach` | Callable from within Pre-Writing or Core DE | Yes |
| `apodictic-audit [name]` | Callable from within any workflow | Yes |
| `apodictic-research [mode]` | Callable from within any workflow | Yes |
...
1. A new `apodictic-start` command file that runs the question sequence
