...
This file is loaded on every `apodictic-start` invocation. It contains the question flow, route map, and gap-handling protocol — everything the LLM needs to execute routing. For design rationale, see `intake-router-design.md`.

---

## Entry Point

`apodictic-start` triggers this router. All existing commands remain functional as direct-access shortcuts (see §5).
...
1. `apodictic-audit argument-red-team` for hostile pressure testing
2. `apodictic-audit argument-evidence` when evidence legitimacy is the likely bottleneck
3. `apodictic-audit argument-persuasion` when audience fit is the likely bottleneck
4. `apodictic-coach` for revision sequencing

---

## §5. Command Shortcuts

All commands work as direct entry points. `apodictic-start` is recommended for new users; the other commands are shortcuts for writers who know what they want.

| Command | What it does | Router equivalent |
|---------|-------------|-------------------|
| `apodictic-start` | Routes to the right workflow in 2–3 questions | The router itself |
| `apodictic-develop-edit` | Run a full development edit | artifact=`full_draft`, goal=`repair` |
| `apodictic-diagnose` | Quick targeted diagnostic | Any artifact, goal=`repair` (targeted) |
| `apodictic-ready` | Submission readiness check | artifact=`full_draft`, goal=`submit` |
| `apodictic-audit [name]` | Run a specific deep-dive analysis | Callable from within workflows |
| `apodictic-research [mode]` | Internet-assisted verification | Callable from within workflows |
| `apodictic-coach` | Post-diagnostic revision coaching | Post-diagnostic (not an intake question) |
| `apodictic-plot-coach` | Plot structure coaching | Callable from within workflows |
| `apodictic-pre-writing` | Idea to draftable structure | artifact=`idea`, goal=`draft` |
| `apodictic-new-project` | Initialize a new project | Initialize project, then run router |
| `apodictic-revision-plan` | *(compatibility alias for `apodictic-coach`)* | — |
...
| partial | repair | time | Submission Triage | Gap: triage requires complete manuscript. Offer targeted `apodictic-diagnose`. |
