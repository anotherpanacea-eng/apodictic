...
1. **Create or confirm the project root** outside the plugin repo. Use Title_Case with underscores for the folder name (e.g., `F1`). If the writer already has an existing output folder in use, reuse it as the project root — create `runs/` inside it if it doesn't exist yet.
...
7. **Report** what was created, where the project root lives, the folder structure, and what the next step is (typically: run `apodictic-develop-edit` or specific passes).
