...
2. **If multi-POV:** build the in-memory JSONL manifest from the POV mapping; invoke `setec_runner.run_supplement("pov_voice_profile.py", ["--manifest", manifest_path, ...], json_out=True)`. Pass `json_out=True`: `pov_voice_profile.py` writes its schema_version 1.0 envelope to a `--json-out <path>` file rather than stdout `--json` (a bare `--json` argparse-prefix-matches to `--json-out` and fails "expected one argument"); the runner allocates the temp path, reads the envelope back, and cleans up. Read `results.pairwise_distances`, `results.pov_vs_corpus`, `results.collapse_verdict`.
