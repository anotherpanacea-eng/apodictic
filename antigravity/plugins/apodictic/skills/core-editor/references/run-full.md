...
2. **If multi-POV:** build the in-memory JSONL manifest from the POV mapping; invoke `setec_runner.run_supplement("pov_voice_profile.py", ["--manifest", manifest_path, ...], json_out=True)`. Pass `json_out=True`: `pov_voice_profile.py` writes its schema_version 1.0 envelope to a `--json-out <path>` file rather than stdout `--json` (a bare `--json` argparse-prefix-matches to `--json-out` and fails "expected one argument"). The runner writes into an ephemeral `ai-prose-baselines-private/` directory — SETEC's default-private output policy refuses voice-cloning outputs anywhere else without the unsafe `--allow-public-output` — then reads the envelope back and removes the tree. (To co-locate the envelope with the writer's real baselines instead, pass an explicit `--json-out <dir>/ai-prose-baselines-private/...` path in the arg list; the runner honors it and leaves it in place.) Read `results.pairwise_distances`, `results.pov_vs_corpus`, `results.collapse_verdict`.
