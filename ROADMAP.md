...
**Status: Built (Increment 1)** — shipped 2026-06-16 (promoted the same day from [Horizon Capacities](#horizon-capacities) Tier 1, item 2; the number is retained there for stable cross-reference). The `annotated-manuscript` validator, the `apodictic.annotation.v1` manifest schema, the CriticMarkup renderer + anchor resolver (`scripts/annotation_manifest.py`), the shared `chapter_token` parser, a canonical `--check-all` fixture, and the reference module all landed (`--self-test-all` 41 → 42, `--check-all` green). Spec: [`docs/annotated-manuscript.md`](docs/annotated-manuscript.md). Future increments (character-precise anchoring; DOCX/GDocs export; letter↔margin cross-links; round-trip re-anchoring) remain.
