...
**In flight (unreleased, 2026-06-20 — an 8-PR stack [#123–#130], all CI-green, awaiting Codex):** the **Horizon Capacities Tier-1 set is now COMPLETE** — Continuity Bible (#7), Uncertainty-Resolution Intake Interview (#18), Author Voice Fingerprint (#9), Content-Advisory (#8), and Reader-Persona Simulation (#5) all built. Plus a **fleet-wide validator hardening**: a 50-finding adversarial sweep closed the **resolver-substring** class across 12 validators and fixed the crash-on-malformed class (4 P1 + 8 non-P1), the softness-downgrade override is now **ID-scoped** (closing a Deficit-Lock gate-bypass), and a new **`validator-conventions` meta-linter** ([§Validator-Conventions](#validator-conventions-meta-linter)) now gates the fleet conventions structurally so the classes cannot recur (validators 49 → 55). See [§Horizon Capacities](#horizon-capacities) for the Tier-1 status table and [§Research-Seeded Candidates](#research-seeded-candidates-2026-06-20) for the new backlog.
...
**Tier 1 status: COMPLETE (2026-06-20).** All nine Tier-1 items plus the reconsidered #18 are built (#1 Manuscript-Structure Visualizations, #2 Annotated-Manuscript, #3 Beta-Reader Instrument, #4 Promise-Contract Fidelity, #5 Reader-Persona Simulation, #6 Draft-over-Draft Regression, #7 Continuity Bible, #8 Content-Advisory, #9 Author Voice Fingerprint, #18 Uncertainty-Resolution Intake Interview). #14 resolved-will-not-build; #15–#17 surfaced-not-viable. **Remaining: Tier 2 only** (#10 Dramatic-Writing Module, #11 Argument-Engine Extension to grants/papers, #12 Songwriting/Lyrics Lens, #13 Standalone Worldbuilding-Bible) — adjacent disciplines / bigger lifts, demand-gated.
...
## Validator-Conventions (meta-linter)

**Status: Built (Increment 1), 2026-06-20.** A validator that validates the validators (`scripts/meta_lint.py`, `validate.sh validator-conventions`), born from a fleet-wide adversarial sweep that found recurring latent bug classes manual review had let drift. It mechanizes the fleet's structural conventions so the classes cannot recur: **M1** every `AGG_VALIDATORS` entry has a `--self-test` dispatcher case, **M2** resolvers classify on parsed blocks (never a raw `"apodictic:<type>" in <text>` substring — the signature anti-pattern that misroutes a file naming a marker in prose), **M3** the count is derived from `AGG_VALIDATORS` (never hand-typed), **M4** no orphan schema. Wired into `--self-test-all` + `--check-all`; itself in `AGG_VALIDATORS`, so M1 checks it against its own rule. Spec: [`docs/validator-conventions.md`](docs/validator-conventions.md).

**The sweep's deferred residue** (tracked; not statically gateable by M1–M4): the lexical-regex over/under-fire heuristics the specs already concede are fuzzy. A future increment could add an AST-level **crash-resistance check** (every `parse_*` guards a non-dict block before `.get()`) — the class fixed by hand in #128/#130 but not yet mechanized.

## Research-Seeded Candidates (2026-06-20)

A deep-research sweep (fan-out web search → 3-vote adversarial verification → synthesis; report at `setec-scratch/arxiv-capability-review/2026-06-20_detection-stylometry-research.md`) surfaced post-2025 advances mapped to buildable proposals. **Most are SETEC-side** (AI-prose detection / voice coherence) and **Tier-gated** (every probability-based detector needs a local scoring LLM — not a stdlib validator), so they belong to the SETEC capability-review pipeline, not APODICTIC core. The **argument-mining bucket returned nothing that survived verification**, so the Nonfiction Argument Engine has **no 2026-fresh provenance** from this set — its dialectical-clarity / warrant / fallacy audits stand on their existing foundations. The one APODICTIC-adjacent thread worth a spec-review pass: **LLM-assisted-but-interpretable stylometric explanation** (latent-space author embeddings labelled with natural-language style features) as a possible *descriptive* overlay on voice-distinctiveness — firewall risk is real (must stay descriptive, never prescriptive). Candidates are unvetted research inputs, not commitments; each needs the spec → review cycle before it earns a Horizon number.
