...
**Status: Built (Increments 1–2)** — Increment 1 shipped 2026-06-16 (margin comments at line-range / section / chapter / document granularity, promoted the same day from [Horizon Capacities](#horizon-capacities) Tier 1, item 2 — number retained there for stable cross-reference). **Increment 2 shipped 2026-06-17:** character-precise *quote-locator* anchoring — the finest `quote` rung, the **A6** quote-integrity gate, a unified character-offset renderer (byte-identical to Increment 1 for non-quote anchors), and the optional `evidence_quote` finding field. Spec: [`docs/annotated-manuscript.md`](docs/annotated-manuscript.md). **Increment 2 is consumer-only:** no shipped diagnostic pass emits `evidence_quote` yet, so the quote rung is *inert on the real corpus* until the **[Producer](#producer--evidence_quote-population)** below lands. Remaining future increments: DOCX / Google-Docs export; letter↔margin cross-links; round-trip re-anchoring.
...
### Producer — `evidence_quote` population

**Roadmapped (Increment 2 follow-on; demand-gated).** Increment 2 built the **consumer** of `evidence_quote` (the quote-locator + the [A6](docs/annotated-manuscript.md) gate); it does not populate the field. For the character-precise `quote` rung to be *live* on real manuscripts, the diagnostic passes that emit `apodictic.finding.v1` blocks must — **when a finding has an exact textual locus** — attach a **verbatim, single-line manuscript substring** as `evidence_quote` (the sentence/clause the finding is about).
...
- **Demand-gated + incremental, not a retrofit.** A pass adopts `evidence_quote` when its findings are *already* sentence-precise — Pass 5 (character beats), Pass 8 (reveal lines), the AI-prose line-level flags, and citation/quote-bearing nonfiction findings are the natural first adopters. Each adoption immediately lights up the quote rung for that pass's output; the rest keep degrading harmlessly. Build per-pass when that pass's output is demonstrably sentence-precise and a writer would benefit from the exact-sentence anchor — never speculatively across all 11 passes at once.
