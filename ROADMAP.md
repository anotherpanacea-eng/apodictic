...
**Remaining (Increment 2–3):** fill buckets 3, 5, 6, 7 with positive controls; add public-domain exemplars (Douglass, *Federalist* No. 10) and private/permission-cleared real fixtures; run the two-independent-runs convergence protocol across the full corpus. ✅ The specced `validate.sh argument-groundtruth-check` validator is **built** — the 17th self-testable validator (`scripts/argument_groundtruth.py`): GT1–GT7 coverage, DC code-namespace resolution, GT2 locus↔code consistency, GT7 Distinguish classification; tuned against the full registered corpus (full + provisional fixtures) and run over it by `--check-all`.
...
**In progress.** Spec + increment plan at [`docs/runner-governed-execution.md`](docs/runner-governed-execution.md). Increments 1–3 are built (cooperative gate manifest + engine + sidecar state + finding-ID lifecycle); increment 4 (external host orchestrator) remains future. Increment 5 — **structured gate-event records** (the re-homed other half of Validator Hardening Track C) — is now **designed**: history-only append-only `execution.gate_events[]` as the canonical record (the v1 per-phase `gates` map dropped as derivable), a stored-event-vs-derived-label lifecycle vocabulary, a thin checkable resume pointer, and a hand-authorable degrade-path event. Build pending; spec in §Increment 5 of the design doc.
...
**In progress.** The ID itself already ships (`apodictic.finding.v1.id`, `F-<ORIGIN>-<NN>`); `structured-findings` owns intra-ledger hygiene and `softness-check` owns severity fidelity (locked→delivered) by ID. Increment 1 (spec + `finding-trace` validator, [`docs/finding-lifecycle-ids.md`](docs/finding-lifecycle-ids.md)) is **built**: `scripts/finding_trace.py` / `validate.sh finding-trace` traces the ID *across* artifacts — the un-owned dimension of cross-artifact **referential integrity + sidecar lifecycle coherence** (E1 dangling letter reference, E2 phantom `finding_states` key, E3 invalid state, W1 lifecycle coverage; `--strict` for CI). The 18th self-testable validator. Increment 2 (revision-plan follow-through + canonical `--check-all` gating) remains future.
