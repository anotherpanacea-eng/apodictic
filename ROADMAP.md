...
**Remaining:** only the **run-folder–shaped** validators are still self-test-only — `gate` / `gate-state` (cooperative gate engine), `escalation-check` (sidecar + ledger), and `argument-recon-prerequisite` (run-folder scan). Gating them needs a canonical *run folder* fixture (ledger + sidecar + gate-event log), not a single canonical file; that is the larger remaining slice. The self-test-only `artifacts-schema` and the contract-shaped `quality-risk-triggers` have no clean single-file canonical target (the latter would need a dedicated clean canonical contract). Defer these until a forcing function (a canonical run-folder fixture lands, or a validator-vs-canonical mismatch surfaces).
